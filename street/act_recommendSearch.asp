...
<div>
	<ul class="brandCate">
		<li style="cursor:pointer;" class="<%= chkiif(cdl="","current","")%>"     onclick="SearchModrecmd('<%=ctab%>', '', '', '',  '<%=scTxt%>','');">전체</li>
		<%=fnBrandStreetCategoryHeaderAct(cdl,"SearchModrecmd",ctab,"",scTxt)%>
	</ul>
	<ul>
		...
		<ul>
			...
			<li>
				<div class="brandPhoto"><a href="/street/street_brand_sub06.asp?makerid=<%=oawardn.FItemList(i).FMakerID%>&gaparam=street_list_<%=i+1%>" target="_blank">
					<img src="<%= oawardn.FItemList(i).FBrandImage %>" width="200" height="200" alt="7321 Design" onerror="this.src = 'https://fiximage.10x10.co.kr/m/2020/common/no_img.svg'" />
				</a></div>
