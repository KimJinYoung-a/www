...
isCrossPlatform = (request("isAx")<>"Y") ''ActiveX IE
	...
	' */
	LGD_RETURNURL				= httpOrSSLTenURL&"/inipay/xpay/returnurl.asp?isAx="&isAx
...
<!--
	...
	function setLGDResult(){
		...
		if(document.getElementById('LGD_RESPCODE').value == '0000' ){
		    <% if (isAx="D") then %>
		    var fdo = window.dialogArguments["opener"].document.frmorder;
		    <% else %>
			...
			<% if (isAx="D") then %>
			window.dialogArguments["opener"].disable_click();
			fdo.submit();
			window.close();
			<% else %>
			fdo.submit();
...
<body >
<!--  수정 불가(IFRAME 방식시 사용)   -->
<div id="LGD_PAYMENTWINDOW" style="position:absolute; display:none; width:480px; height:480px; background-color:#D3D3D3; font-size:small; overflow:visible;">
