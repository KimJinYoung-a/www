...
<%
Dim CST_PLATFORM, CST_MID, LGD_MID

CST_MID					   = "tenbyten02"						'상점아이디(LG유플러스으로 부터 발급받으신 상점아이디를 입력하세요)
IF application("Svr_Info") = "Dev" THEN
	CST_PLATFORM = "test"
	LGD_MID = "t" & CST_MID    '테스트 아이디는 't'를 반드시 제외하고 입력하세요.
Else
...
vIDx = fnSaveOrderTemp(LGD_MID, iErrMsg, ipgGubun, irefPgParam)  '' order_temp 임시저장

if (vIDx<1) then
    response.write "<script>alert('처리중 오류가 발생하였습니다.\r\n- "&replace(iErrMsg,"'","")&"');opener.location.href='/inipay/shoppingbag.asp';window.close();</script>"
    dbget.close()
    response.end
end if
...
isCrossPlatform = (request("isAx")<>"Y") ''ActiveX IE
	...
	LGD_AMOUNT                 = trim(request("mobileprdprice"))         '결제금액("," 를 제외한 결제금액을 입력하세요)
	LGD_MERTKEY                = "04986fbf874cc8e6affa02f165f6b4f2"	 '[반드시 세팅]상점MertKey(mertkey는 상점관리자 -> 계약정보 -> 상점정보관리에서 확인하실수 있습니다')
	LGD_BUYER                  = trim(request("buyname"))          '구매자명
	LGD_PRODUCTINFO            = trim(request("mobileprdtnm"))    '상품명
	LGD_BUYEREMAIL             = trim(request("buyemail"))     '구매자 이메일
	...
	LGD_CUSTOM_FIRSTPAY        = "SC0060"                           '상점정의 초기결제수단
	LGD_CUSTOM_SKIN            = "red"                               '상점정의 결제창 스킨 (red, blue, cyan, green, yellow)

    '/*
	' * LGD_RETURNURL 을 설정하여 주시기 바랍니다. 반드시 현재 페이지와 동일한 프로트콜 및  호스트이어야 합니다. 아래 부분을 반드시 수정하십시요.
	' */
	LGD_RETURNURL				= httpOrSSLTenURL&"/inipay/xpay/returnurl_tmp.asp?isAx="&isAx
	...
	' */
	LGD_HASHDATA = LCASE(md5( LGD_MID & LGD_OID & LGD_AMOUNT & LGD_TIMESTAMP & LGD_MERTKEY ))  ''LowerCase 로 체크함.2017/12/19
	...
	' */
	Dim userphone
	userphone                    = trim(request("buyhp1")&request("buyhp2")&request("buyhp3"))
...
<input type="hidden" name="vidx" id="vidx">
</form>
</body>
<%
if (isCrossPlatform) then
