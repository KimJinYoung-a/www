...
<div class="footer-wrap">
	<div class="foot-nav">
		<ul>
			...
			<li><a href="<%=SSLUrl%>/common/join.asp"><strong>이용약관</strong></a></li>
			<li><a href="<%=SSLUrl%>/common/private.asp"><strong>개인정보 처리방침</strong></a></li>
			<li><a href="<%=SSLUrl%>/common/youth.asp"><strong>청소년 보호정책</strong></a></li>
			...
			<!-- 다스배너 -->
<!--			<li class="diary notice"><a href="<%=SSLUrl%>/diarystory2021/index.asp"><span>NOTICE</span>2021 다이어리 스토리</a></li>-->
		</ul>
	</div>
	<div class="footerV18">
...
<% If IsUserLoginOK Then %>
	<% If now() > #07/09/2018 10:00:00# And Now() < #07/16/2018 00:00:00# then %>
		<% 	If request.cookies("appcoupon1060")<>"Y" Then %>
			...
			.front-Bnr-appCouponBanner {position:fixed; left:50%; top:50%; z-index:99999; margin:-303px 0 0 -243px;}
			.front-Bnr-appCouponBanner .btnClose {position:absolute; top:-25px; left:50%; margin-left:217px; width:52px; height:52px;}
			.front-Bnr-appCouponBanner .btnClose button{width:100%; height:100%; border:0; background-color:transparent; text-indent:-999em;}
			</style>
			<div class="front-Bnr-appCouponBanner" style="display:none;">
				<p><a href="<%=SSLUrl%>/my10x10/couponbook.asp"><img src="http://webimage.10x10.co.kr/eventIMG/2018/mkt/0709/bnr_front.jpg" alt="첫 구매 쿠폰이 발급되었습니다 App Coupon 5000원 24시간 이내 쿠폰함으로 가기" /></a></p>
				<div class="btnClose"><button type="button" onclick="window.close();" style="background-image:url(http://webimage.10x10.co.kr/eventIMG/2018/mkt/0709/btn_close.png)">닫기</button></div>
			</div>
		<% End If %>
	<% End If %>
<% End If %>
...
<% IF application("Svr_Info") <> "Dev1" THEN %>
	...
	<script type="text/javascript">
		<%
			if trim(kakaoAnal_AddScript)<>"" then
				response.write "try { "
				response.write " kakaoPixel('6348634682977072419').pageView(); "
				response.write kakaoAnal_AddScript
				response.write " } catch (error) { "
				response.write " console.log(error.name) "
				response.write " } "
			end if
		%>
	</script>
	...
	<%' Facebook %>
<% if Instr(Request.ServerVariables("HTTP_USER_AGENT"), "Chrome/75") < 1 then %>
	...
	<script type="text/javascript">
		...
		<% If IsUserLoginOK Then %>
			<% If Trim(session("appboySession")) <> "" Then %>
				...
				amplitude.getInstance().init('91316725130cbc5b997cef756ce9388a', '<%=Trim(session("appboySession"))%>', {
					includeUtm : true,
