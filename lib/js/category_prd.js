	...
	$(document).ready(function () {
		...
		// 플러스할인 담기
		$('.plusSaleBoxV15 a[name="btnPlus"]').click(function(e) {
			e.preventDefault();
			var tFrm = $(this).parent().parent();
			...
			var sAddItem='';
			sAddItem += '<tr>';
			sAddItem += '	<td class="lt">'
			sAddItem += '		<p class="plusPdtOrder">' + itemNm + '</p>';
			if($(tFrm).has('textarea[name="requiredetailplus"]').length) {
				sAddItem += '<p class="tPad05"><textarea name="optRequire" style="width:215px; height:35px;">'+ $(tRqr).val() +'</textarea></p>';
			} else {
				sAddItem += '<input type="hidden" name="optRequire" value="" />';
			}
			...
			sAddItem += '</td>';
			sAddItem += '	<td><input type="text" id="optItemEa" /></td>';
			sAddItem += '	<td class="rt rPad05">' + plusComma(plusPrc+optAddPrc) + '</td>';
			sAddItem += '	<td><a href="" class="del"><span class="btnListDel">삭제</span></a></td>';
			sAddItem += '</tr>';
			...
			// 스피너 변환
			$("#optItemEa").numSpinner({min:1,max:opLimit,step:1,value:1});
			...
			if($("#lySpBagList").find("tr").length>0) {
				...
				// 선택창 옵션 초기화
				$(tFrm).find('[name="item_option"]').val("");
	...
	function FnAddShoppingBag(bool) {
	    ...
	    var amplitudeFrm = document.amplitudeFrm;
	    var optCode = "0000";
				...
				success: function(message) {
					switch(message.split("||")[0]) {
						...
						case "1":
							...
							let values = amplitudeFrm.brand_id.value + '|' + amplitudeFrm.brand_name.value + '|' + amplitudeFrm.category_name.value + '|' + amplitudeFrm.item_id.value + '|' + amplitudeFrm.product_name.value;
							fnAmplitudeEventMultiPropertiesAction('click_shoppingbag_in_product', keys, values);
							break;
						case "2":
