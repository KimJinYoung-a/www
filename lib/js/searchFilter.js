...
function delFilterItem(obj) {
	...
	if(sId=="PrcRng") {
		...
		//컬러검사
		$("#"+sId).parents(1).removeClass('selected');
...
function setSearchDiaryFilterItem() {
	...
	if($("#diaryAttribute li input:checked").length) {
		var tmA = $("#diaryAttribute li input:checked").first().attr("value").substr(0,3);
		sFtCont += "<dl>"
		sFtCont += "<dt>"+ $("#diaryAttribute li input:checked").first().parent().find("label").attr("prv") +"</dt>"
