...
class cevent
	...
	public FRectKind
	public frectnotinevt_code

	'/inc_bestitem_event.asp
	public sub GetBrandEventList()
		dim sqlStr, i

		sqlStr =	"exec db_user.dbo.sp_Ten_BrandStreet_Event " & FpageSize & ",'" & FRectMakerid & "', '" & FRectKind & "', '"& frectnotinevt_code &"'"
		...
		if  not rsget.EOF  then
			do until rsget.eof
				...
				FItemList(i).FECode	= rsget("evt_code")
				FItemList(i).FEName	= db2html(rsget("evt_name"))
				FItemList(i).FEKind	= rsget("evt_kind")
				FItemList(i).FESDate	= rsget("evt_startdate")
				FItemList(i).FEEDate	= rsget("evt_enddate")
				FItemList(i).Fevt_bannerimg	= db2html(rsget("evt_bannerimg"))
				FItemList(i).Fevt_bannerimg2010	= db2html(rsget("evt_bannerimg2010"))
				FItemList(i).Fevt_newest = db2html(rsget("evt_newest"))
				FItemList(i).Fevt_bannerlink = db2html(rsget("evt_bannerlink"))
				FItemList(i).fetc_itemid = rsget("etc_itemid")
				FItemList(i).fetc_itemimg = rsget("etc_itemimg")
				FItemList(i).fbasicimage600 = rsget("basicimage600")
				FItemList(i).fbasicimage = rsget("basicimage")
				FItemList(i).fevt_subcopyK = rsget("evt_subcopyK")
		...
		If  not rsget.EOF  then
			...
			Do until rsget.eof
				Set FItemList(i) = new CCategoryPrdItem
					...
					FItemList(i).FImageBasic 		= "http://webimage.10x10.co.kr/image/basic/" + GetImageSubFolderByItemid(FItemList(i).FItemid) + "/" + db2html(rsget("basicimage"))
		...
		If  not rsget.EOF  then
			...
			Do until rsget.eof
				Set FItemList(i) = new CCategoryPrdItem
					...
					FItemList(i).flastadminid			= rsget("lastadminid")
					if rsget("icon1image") <> "" then
						FItemList(i).FImageBasic = "http://webimage.10x10.co.kr/image/icon1/" & GetImageSubFolderByItemid(rsget("itemid")) & "/" & db2html(rsget("icon1image"))
					end if
					if rsget("icon2image") <> "" then
						FItemList(i).FImageBasic = "http://webimage.10x10.co.kr/image/icon2/" & GetImageSubFolderByItemid(rsget("itemid")) & "/" & db2html(rsget("icon2image"))
					end if
					if rsget("basicimage") <> "" then
						FItemList(i).FImageBasic = "http://webimage.10x10.co.kr/image/basic/" & GetImageSubFolderByItemid(rsget("itemid")) & "/" & db2html(rsget("basicimage"))
					end if
					FItemList(i).fEvalCnt			= rsget("EvalCnt")
					FItemList(i).FEvalcnt_Photo			= rsget("Evalcnt_Photo")
					FItemList(i).ffavCount			= rsget("favCount")
		...
		If not rsget.EOF then
			Do until rsget.EOF
				Set FOneItem = new cHello_item
					...
					If Not IsNull(FOneItem.FBgImageURL) And Len(Trim(FOneItem.FBgImageURL)) > 2 Then
						Dim bgImageType, modelitem
						bgImageType = Left(FOneItem.FBgImageURL, 2)
						modelitem = rsget("modelitem")
						FOneItem.FBgImageURL = fnGetBrandImage(modelitem, FOneItem.FBgImageURL)
					Else
						FOneItem.FBgImageURL = ""
					End If
				i = i + 1
				rsget.moveNext
			Loop
...
function getbrandinfo(makerid)
	...
	sqlstr = "select top 1"
	sqlstr = sqlstr & " c.userid, c.socname, c.socname_kor, c.itemcount, c.defaultFreeBeasongLimit, c.defaultDeliverPay"
	sqlstr = sqlstr & " from [db_user].dbo.tbl_user_c c"
	sqlstr = sqlstr & " where c.isusing='Y'"
	sqlstr = sqlstr & " and c.userid='"& makerid &"'"
...
function Getshop_event_one(imakerid, ievt_kind)
	dim SqlStr, tmpGetshop_event_one, tmpevt_code
	if imakerid="" then
		Getshop_event_one = ""
		exit function
	end if
	...
	Getshop_event_one = tmpGetshop_event_one & "!@#" & tmpevt_code
End function
...
'//나의 찜 브랜드 보기 2
Function getMyzzimBrand()
	Dim sqlStr
	If getloginuserid() <> "" Then
		
		response.write "<dd style=""display: none;""><ul>"
		...
		If not rsget.EOF then
			Do until rsget.EOF
				response.write "<li onclick=""GoMyBrand('"& rsget("makerid") &"');"" >" & db2html(rsget("socname_kor")) & "</li>"
				rsget.movenext
			Loop
		End If
		rsget.close
	...
	Else
		...
		response.write "<ul><li>로그인이 필요합니다.</li></ul>"
		response.write "</dd>"
		Exit Function
	End If
End Function
