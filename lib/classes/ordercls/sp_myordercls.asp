...
class CMyOrderMasterItem
    ...
    public function IsRentalOrder
        IsRentalOrder = (Fjumundiv="8")
    end function
    ...
    '' 해외 직구 통관번호 수정 가능한 상태인가? :: 상품준비중(업체확인 이후) 고객수정불가
    public function isUniPassNumberEditEnable(idetail)
        ...
        if isArray(idetail.FItemList) then
            for i=LBound(idetail.FItemList) to UBound(idetail.FItemList)
                if Not (isEmpty(idetail.FItemList(i))) then
        			if Not (idetail.FItemList(i) is Nothing) then
        				if (idetail.FItemList(i).IsGlobalDirectPurchaseItem) and (idetail.FItemList(i).Fcurrstate>=3) then
...
'    end function
    ...
    ''유료 사은품 상품 체크
    public function IsChargeFreebiesItemExistsCheck(idetail)
        IsChargeFreebiesItemExistsCheck = true
        dim i
        ...
        if isArray(idetail.FItemList) then
            for i=LBound(idetail.FItemList) to UBound(idetail.FItemList)
                if Not (isEmpty(idetail.FItemList(i))) then
        			if Not (idetail.FItemList(i) is Nothing) then
        				if (idetail.FItemList(i).Fplus_sale_item_idx>0) then
        					IsChargeFreebiesItemExistsCheck = false
        					Exit function
        				end if
        			end if
        		end if
...
Class CMyOrder
	...
	public Sub getMainPaymentInfo(byval paymethod, byref orgpayment, byref cardcancelok, byref cardcancelerrormsg, byref cardcancelcount, byref cardcancelsum, byref cardcode)
		...
		sqlStr = sqlStr + " 	and e.acctdiv in ('7', '100', '550', '560', '20', '50', '80', '90', '400', '110', '150') "							'OK CASH BAG 은 주결제수단이다.
    ...
    public Sub GetMyOrderList()
		...
		if FrectSiteName<>"" then
            if (FrectSiteName = "10x10") then
                sqlStr = sqlStr + " and m.sitename in ('10x10', '10x10_cs') " & vbCrLf
            else
...
'### 이니렌탈의 매출전표를 위한 aes128 tid 암호화값 가져오기
function fnGetIniRentalAesEncodeTid(byval tid, byref strData, byref iniRentalMid)
...
'//보너스쿠폰 번호 가져오기
Function fnGetBonuscouponidx(orderserial)
	Dim vQuery, vTemp
	vQuery = "SELECT TOP 1 bonuscouponidx FROM [db_order].[dbo].[tbl_order_detail] WITH(NOLOCK) "
	vQuery = vQuery & "where orderserial = '" & orderserial & "'"
	vQuery = vQuery & "AND itemid <> 0 AND cancelyn <> 'Y' AND bonuscouponidx IS NOT NULL"
	rsget.CursorLocation = adUseClient
	rsget.Open vQuery, dbget, adOpenForwardOnly, adLockReadOnly
	if not rsget.eof then
		vTemp = rsget(0)
