...
'		Response.end
	...
	Public Function fnGetPCMultiAddBanner
		Dim strSql
		If FMenuIDX = "" THEN Exit Function
		strSql ="EXEC [db_event].[dbo].[usp_WWW_Event_PCAddbanner_Multi_Get] "&FMenuIDX&""
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly, adCmdText
		IF Not (rsget.EOF OR rsget.BOF) THEN
...
Function sbMultiContentsSlidetemplate(MenuIDX)
	...
	If cEventadd.FSidx <> "" Then 
%>
	<div class="slideTemplateV15 <%=gubuncls%>" id='multi<%=MenuIDX%>'>
		<% If cEventadd.FStopimg <> "" Then %>
		<div class="evtTop" <% If cEventadd.FStopaddimg <> "" Then %>style="background-image:url(<%=cEventadd.FStopaddimg%>);"<% End If %>>
			<img src="<%=cEventadd.FStopimg%>" alt="" />
		</div>
...
<div class="full-contV19<% If textbgcolor<>1 Then %> blk<% else %> wht<% end if %>"<% if newGiftBox="" then %> id="topslide"<% end if %>>
	<div class="rolling" id='multi<%=MenuIDX%>'>
		<div class="slider">
		...
		For intSL = 0 To UBound(vSArray,2)
			if vSArray(1,intSL)="1" then
				if vSArray(5,intSL)<>"" then
					AddContents = "<div class='evt-sliderV19' style='background-image:url(" & vSArray(5,intSL) & ");margin-top:" & vSArray(9,intSL) & "px;' id='multi"&Cstr(vSArray(0,intSL))&"'>" & vbcrlf
				else
					AddContents = "<div class='evt-sliderV19' style='margin-top:" & vSArray(9,intSL) & "px;' id='multi"&Cstr(vSArray(0,intSL))&"'>" & vbcrlf
				end if
			...
			elseif vSArray(1,intSL)="2" then
				if vSArray(5,intSL)<>"" then
					AddContents = "<div class='evt-vod' style='background-image:url(" & vSArray(5,intSL) & ");margin-top:" & vSArray(9,intSL) & "px;' id='multi"&Cstr(vSArray(0,intSL))&"'>" & vbcrlf
				else
					AddContents = "<div class='evt-vod' style='margin-top:" & vSArray(9,intSL) & "px;' id='multi"&Cstr(vSArray(0,intSL))&"'>" & vbcrlf
				end if
				AddContents = AddContents & sbMultiContentsDetail(vSArray(0,intSL), vSArray(1,intSL), vSArray(3,intSL))
				if vSArray(6,intSL)<>"" then
				AddContents = AddContents & "	<div class='bg-color left' style='background-color:" & vSArray(6,intSL) & ";'></div>" & vbcrlf
			...
			elseif vSArray(1,intSL)="3" then
				if vSArray(5,intSL)<>"" then
					AddContents = "<div class='evt-brandV18' style='background-image:url(" & vSArray(5,intSL) & ");margin-top:" & vSArray(9,intSL) & "px;' id='multi"&Cstr(vSArray(0,intSL))&"'>" & vbcrlf
				else
					AddContents = "<div class='evt-brandV18' style='margin-top:" & vSArray(9,intSL) & "px;' id='multi"&Cstr(vSArray(0,intSL))&"'>" & vbcrlf
				end if
				AddContents = AddContents & sbMultiContentsDetail(vSArray(0,intSL), vSArray(1,intSL), vSArray(3,intSL))
				if vSArray(6,intSL)<>"" then
				AddContents = AddContents & "	<div class='bg-color left' style='background-color:" & vSArray(6,intSL) & ";'></div>" & vbcrlf
			...
			elseif vSArray(1,intSL)="4" then
				...
				if vSArray(5,intSL)<>"" then
					AddContents = "<div class='evt-prdV18 " & txtcolor & "' style='background-image:url(" & vSArray(5,intSL) & ");margin-top:" & vSArray(9,intSL) & "px;' id='multi"&Cstr(vSArray(0,intSL))&"'>" & vbcrlf
				else
					AddContents = "<div class='evt-prdV18 " & txtcolor & "' style='margin-top:" & vSArray(9,intSL) & "px;' id='multi"&Cstr(vSArray(0,intSL))&"'>" & vbcrlf
				end if
			...
			elseif vSArray(1,intSL)="5" then
				if vSArray(5,intSL)<>"" then
					AddContents = "<div class='evt-descV19' style='background-image:url(" & vSArray(5,intSL) & ");margin-top:" & vSArray(9,intSL) & "px;' id='multi"&Cstr(vSArray(0,intSL))&"'>" & vbcrlf
				else
					AddContents = "<div class='evt-descV19' style='margin-top:" & vSArray(9,intSL) & "px;' id='multi"&Cstr(vSArray(0,intSL))&"'>" & vbcrlf
				end if
	...
	If (iTotCnt >= 0) Then
		IF GroupItemViewType = "A" OR GroupItemViewType = "B" THEN '// A or B
			For intSL=0 To iTotCnt
				...
				ArrContents = ArrContents & "<div class='evt-itemV20 "& chkiif( GroupItemViewType = "A" , "typeA" , "typeB" ) &"'>"
	...
	elseif MenuDIV="11" then
		...
		If isArray(vSArray) THEN
			For intSL = 0 To UBound(vSArray,2)
				...
				elseif MenuDIV="7" then
					if vSArray(0,intSL)<>"" then
					ArrContents = ArrContents & "<img src='" & vSArray(0,intSL) & "' usemap='#Mainmap' class='gpimg' />" & vbcrlf
					end if
					ArrContents = ArrContents & vSArray(1,intSL) & vbcrlf
				elseif MenuDIV="9" then
					If vSArray(1,intSL) <> "" Then
						If (CStr(Date()) >= CStr(vSArray(4,intSL)) and CStr(Date()) <= CStr(vSArray(5,intSL))) Then
