...
Sub printCategoryHistory_B(byRef code)
	...
	if NOT(rsMem.EOF or rsMem.BOF) then
		If Not(isNull(rsMem(0))) Then
			for i = 1 to j
				strHistory = strHistory & "&nbsp;&gt;&nbsp;<a href=""" & SSLUrl & "/shopping/category_list.asp?disp="&Left(code,(3*i))&""">"
				if i = j then
	...
	if trim(strHistory)<>"" then
		Response.Write "<a href='" & SSLUrl & "/'>HOME</a>" & strHistory
	else
		code=0
	end if
end Sub
...
FUNCTION getCateName(byRef code, depth)
    ...
    'response.write code & "---" & lenght & "---"
    'response.write rsMem(0) & "---"
    'response.end
...
Function CategorySelectBoxOption(code)
	...
	vBody = vBody & "<option value=""104"" " & CHKIIF(code="104","selected","") & ">토이/취미</option>"	
...
Function fnAwardBestCategoryLI(code,url)
	Dim vBody
	vBody = ""
	vBody = vBody & "<li class=""" & chkIIF(code="101","on","") & """><a href=""" & url & "disp=101"">디자인문구</a></li>" & vbCrLf
	vBody = vBody & "<li class=""" & chkIIF(code="102","on","") & """><a href=""" & url & "disp=102"">디지털/핸드폰</a></li>" & vbCrLf
	'vBody = vBody & "<li class=""" & chkIIF(code="103","on","") & """><a href=""" & url & "disp=103"">캠핑/트래블</a></li>" & vbCrLf
	vBody = vBody & "<li class=""" & chkIIF(code="124","on","") & """><a href=""" & url & "disp=124"">디자인가전</a></li>" & vbCrLf
	vBody = vBody & "<li class=""" & chkIIF(code="121","on","") & """><a href=""" & url & "disp=121"">가구/수납</a></li>" & vbCrLf
	vBody = vBody & "<li class=""" & chkIIF(code="120","on","") & """><a href=""" & url & "disp=120"">패브릭/생활</a></li>" & vbCrLf	
	vBody = vBody & "<li class=""" & chkIIF(code="122","on","") & """><a href=""" & url & "disp=122"">데코/조명</a></li>" & vbCrLf
	vBody = vBody & "<li class=""" & chkIIF(code="112","on","") & """><a href=""" & url & "disp=112"">키친</a></li>" & vbCrLf
	vBody = vBody & "<li class=""" & chkIIF(code="119","on","") & """><a href=""" & url & "disp=119"">푸드</a></li>" & vbCrLf
	'vBody = vBody & "<li class=""" & chkIIF(code="115","on","") & """><a href=""" & url & "disp=115"">베이비/키즈</a></li>" & vbCrLf	
	vBody = vBody & "<li class=""" & chkIIF(code="117","on","") & """><a href=""" & url & "disp=117"">패션의류</a></li>" & vbCrLf
	vBody = vBody & "<li class=""" & chkIIF(code="116","on","") & """><a href=""" & url & "disp=116"">패션잡화</a></li>" & vbCrLf
	vBody = vBody & "<li class=""" & chkIIF(code="118","on","") & """><a href=""" & url & "disp=118"">뷰티</a></li>" & vbCrLf	
	vBody = vBody & "<li class=""" & chkIIF(code="125","on","") & """><a href=""" & url & "disp=125"">주얼리/시계</a></li>" & vbCrLf
	vBody = vBody & "<li class=""" & chkIIF(code="110","on","") & """><a href=""" & url & "disp=110"">Cat&amp;Dog</a></li>" & vbCrLf
	vBody = vBody & "<li class=""" & chkIIF(code="104","on","") & """><a href=""" & url & "disp=104"">토이/취미</a></li>" & vbCrLf
	vBody = vBody & "<li class=""" & chkIIF(code="103","on","") & """><a href=""" & url & "disp=103"">캠핑</a></li>" & vbCrLf

	fnAwardBestCategoryLi = vBody
End Function

function fnSelectSoringName(v)
	select case v
		case "ne" : fnSelectSoringName = "신상품순"
		case "bs" : fnSelectSoringName = "판매량순"
		case "be" : fnSelectSoringName = "인기상품순"
		case "hp" : fnSelectSoringName = "높은가격순"
		case "lp" : fnSelectSoringName = "낮은가격순"
		case "hs" : fnSelectSoringName = "높은할인율순"
	end select									
end function

function fnSelectEventSortingName(v)
	select case v
		case "0" : fnSelectEventSortingName = "최근이벤트순"
		case "1" : fnSelectEventSortingName = "마감임박순"
		case "2" : fnSelectEventSortingName = "판매순"
		case "3" : fnSelectEventSortingName = "할인율순"
	end select	
end function 

function fnSelectCategoryName(disp)
	select case disp
		case "101" : fnSelectCategoryName = "디자인문구"
		case "102" : fnSelectCategoryName = "디지털/핸드폰"
		case "103" : fnSelectCategoryName = "캠핑"
		case "104" : fnSelectCategoryName = "토이/취미"
		case "124" : fnSelectCategoryName = "디자인가전"
		case "121" : fnSelectCategoryName = "가구/수납"
		case "122" : fnSelectCategoryName = "데코/조명"
		case "120" : fnSelectCategoryName = "패브릭/생활"
		case "112" : fnSelectCategoryName = "키친"
		case "119" : fnSelectCategoryName = "푸드"
		case "117" : fnSelectCategoryName = "패션의류"
		case "116" : fnSelectCategoryName = "패션잡화"
		case "125" : fnSelectCategoryName = "주얼리/시계"
		case "118" : fnSelectCategoryName = "뷰티"
		case "115" : fnSelectCategoryName = "베이비/키즈"
		case "110" : fnSelectCategoryName = "Cat&amp;Dog"
	case else
		fnSelectCategoryName = "전체 카테고리"
	end select									
end function
...
Function fnAwardBestCategoryHeaderSSL()
	...
	vBody = vBody & "<li><p onClick=""location.href='"&wwwUrl&"/shopping/category_main.asp?disp=104';"">토이/취미</p></li>" & vbCrLf	
...
Function fnBrandStreetCategoryHeaderAct(code,jsname,charcd,Lang,scTxt)
	...
	vBody = vBody & "<li style=""cursor:pointer;"" " & chkiif(code="104","class='current'","") & " onclick="""&jsname&"('"&charcd&"', '"&Lang&"', '104', '', '"&scTxt&"'"&vPage&");"">토이/취미</li>" & vbCrLf
	vBody = vBody & "<li style=""cursor:pointer;"" " & chkiif(code="103","class='current'","") & " onclick="""&jsname&"('"&charcd&"', '"&Lang&"', '103', '', '"&scTxt&"'"&vPage&");"">캠핑</li>" & vbCrLf
