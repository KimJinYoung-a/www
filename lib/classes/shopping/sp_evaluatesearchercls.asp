...
Class CEvaluateSearcher
	...
	Public Sub getNotEvaluatedItem()
		...
		sqlStr = " " &_
			...
			" , dci.catecode, first_depth_cate.catename AS first_depth_cate, second_depth_cate.catename AS second_depth_cate  " &_
			" FROM db_order.[dbo].tbl_order_master m WITH(READUNCOMMITTED) " &_
			" JOIN db_order.[dbo].tbl_order_detail d WITH(READUNCOMMITTED) " &_
			...
			" JOIN db_item.[dbo].tbl_item i WITH(READUNCOMMITTED) " &_
			" 	on d.itemid=i.itemid  " &_
			" LEFT OUTER JOIN db_item.[dbo].tbl_display_cate_item AS dci WITH(READUNCOMMITTED) ON d.itemid=dci.itemid " &_
            " LEFT OUTER JOIN [db_item].[dbo].tbl_display_cate AS first_depth_cate WITH(NOLOCK) ON LEFT(dci.catecode, 3) = first_depth_cate.catecode " &_
            " LEFT OUTER JOIN [db_item].[dbo].tbl_display_cate AS second_depth_cate WITH(NOLOCK) ON LEFT(dci.catecode, 6) = second_depth_cate.catecode " &_
			" LEFT JOIN db_item.[dbo].tbl_item_option o WITH(READUNCOMMITTED) " &_
			" 	on d.itemid=o.itemid and d.itemoption = o.itemoption  " &_
			" left join db_board.[dbo].tbl_item_evaluate e WITH(READUNCOMMITTED)" &_
			...
			IF not rsget.eof THEN
                    ...
                    FEvalItem.Fcatecode = rsget("catecode")
                    FEvalItem.Ffirst_depth_cate = rsget("first_depth_cate")
                    FEvalItem.Fsecond_depth_cate = rsget("second_depth_cate")
			END IF
	...
	Public Sub getNotEvaluatedOffShopItem()
		...
		sqlStr = sqlStr + " LEFT JOIN [db_sitemaster].[dbo].tbl_100proshop s WITH(READUNCOMMITTED) " & vbcrlf
...
'				FItemList(i).Fitemid    = rsget("itemid")
	...
	Public Sub getEvaluatedItem_cnt()
		...
		if FRectOption="" then
			...
			sqlStr = sqlStr & " from "
			sqlStr = sqlStr & " [db_item].[dbo].tbl_item WITH(READUNCOMMITTED)"
			...
			sqlStr = sqlStr & " from [db_item].[dbo].tbl_item as I WITH(READUNCOMMITTED)"
			...
			sqlStr = sqlStr & " 		from db_board.dbo.tbl_Item_Evaluate WITH(READUNCOMMITTED)"
			...
			sqlStr = sqlStr & " 		from [db_event].dbo.tbl_tester_Item_Evaluate WITH(READUNCOMMITTED)"
			sqlStr = sqlStr & " 		where itemid = " & FRectItemID & " "
			sqlStr = sqlStr & " 		group by itemid "
			sqlStr = sqlStr & " 	) as T "
	...
	public sub getItemEvalList_itemid()
		...
		sqlStr = " "&_
					 ...
					 " FROM [db_board].[dbo].[tbl_Item_Evaluate] WITH(READUNCOMMITTED)" &_
					 " WHERE IsUsing='Y' " &_
