...
Class SearchItemCls
		...
		IF FarrCate<>"" THEN
			...
			for lpCt=0 to ubound(arrCt)
				if FRectSearchCateDep="X" then
					...
					strQue = strQue & " idx_catecode like '" & getNumeric(RequestCheckVar(LCase(trim(arrCt(lpCt))),18)) & "*' "
				end if
	...
	PUBLIC SUB getSearchList()
        ...
        if (Scn= "scn_dt_itemDispColor") or (FRectMakerid<>"") then
        	...
        	SvrAddr = getTimeChkAddr(G_4THSCH_ADDR)  ''G_3RDSCH_ADDR
        end if
        ''---------------------------------------------------------------------------------------------------------
...
CALL debugQuery(Docruzer,Scn,SearchQuery,SortQuery,FTotalCount,FResultcount)
	...
	PUBLIC SUB getTotalCount()
        ...
        ''칼라칩검색/브랜드검색 3차
        if (Scn= "scn_dt_itemDispColor") or (FRectMakerid<>"") then SvrAddr = getTimeChkAddr(G_4THSCH_ADDR) ''G_3RDSCH_ADDR
        ''---------------------------------------------------------------------------------------------------------
