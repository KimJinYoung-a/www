...
Class SearchItemCls
	...
	Dim FRectDiaryItem		'검색 시 다이어리 아이템 체크
	...
	FUNCTION getSearchQuery(byref query)
		...
		IF FRectDiaryItem<>"" THEN
			strQue= strQue & getQrCon(strQue) & "idx_giftdiv>0"
		END IF
	...
	PUBLIC SUB getSearchList()
        ...
        if (StartNum<0) then StartNum=0 ''2018/07/10 추가
		...
		IF (FLogsAccept) and (FRectSearchTxt<>"") and (FCurrPage="1") THEN
            ...
            Dim iLOG_USER : iLOG_USER = GetUserLevelStr(GetLoginUserLevel) '' 회원등급을 사용 ==>2019/11/06 변경.
			...
			SET FItemList(iRows) = NEW CCategoryPrdItem
				...
				if (FcolorCode="" or FcolorCode="0") Then
					If FItemList(iRows).FItemDiv="21" Then
						if instr(db2html(arrData(11)),"/") > 0 then
				...
				If FItemList(iRows).FItemDiv="21" Then
					If arrData(53)<>"" Then
						if instr(arrData(53),"/") > 0 then
							FItemList(iRows).FImageBasic600 	= getItemImageUrl & "/basic600/" & arrData(53)
						Else
							FItemList(iRows).FImageBasic600 	= getItemImageUrl & "/basic600/" & GetImageSubFolderByItemid(FItemList(iRows).FItemid) & "/" &arrData(53)
						End If
					Else
						FItemList(iRows).FImageBasic600 = FItemList(iRows).FImageBasic
					End If
				Else
					If arrData(53)<>"" Then
						FItemList(iRows).FImageBasic600 	= getItemImageUrl & "/basic600/" & GetImageSubFolderByItemid(FItemList(iRows).FItemid) & "/" &arrData(53)
				...
				FItemList(iRows).Fiskimtentenrecom = arrData(74)
			SET arrData = NOTHING
			SET arrSize = NOTHING
...
Class SearchItemEvaluate
	...
	public Sub GetBestReviewArrayList()
		...
		'// 해외직구배송작업추가
		sql = sql & " , i.deliverfixday, i.adultType, i.itemdiv"
		...
		if  not rsget.EOF  then
			...
			Do Until rsget.EOF or rsget.BOF
				...
				If rsget("itemdiv")="21" Then
					if instr(rsget("listimage120"),"/") > 0 then
						FItemList(i).FImageList120 = "http://webimage.10x10.co.kr/image" & "/list120/" & rsget("listimage120")
						FItemList(i).FIcon1Image = "http://webimage.10x10.co.kr/image" & "/icon1/" & rsget("icon1image")
					Else
...
Class SearchEventCls
	...
	PUBLIC SUB getEventListMore()
		...
		SearchQuery = SearchQuery &_
					...
					" and not idx_evt_code in (" & CStr(FRectEvtCode) & ")"
		Select Case FRectChannel
			Case "W"
				SearchQuery = SearchQuery & " and idx_evt_isWeb=1 "
			Case "M"
...
'' 검색추가로그 = 상품클릭
public Sub AddSearchAddLogItemClick()
    Dim SvrAddr : SvrAddr = getTimeChkAddr(G_2NDSCH_ADDR)
...
Public Function fnRentalPriceCalculationDataInEventList(p)
	dim rentalPee
	...
	If now() >= #2021-05-03 09:00:00# and now() < #2021-06-01 00:00:00# Then
		...
		rentalPee = 1.105
		fnRentalPriceCalculationDataInEventList = Fix(((p*rentalPee) / 12)/100)*100
	Else
