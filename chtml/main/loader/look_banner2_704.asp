...
Dim image, image2, link, linktext, linktext2, startdate, enddate, linktext3
	sFolder = "/chtml/xml/"
	mainFile = "mainXMLBanner_704.xml"
...
If fileCont<>"" Then
	Response.Write "<ul class='lookListV15'>"
	...
	For each tplNodes in cTmpl
		...
		if i>7 then exit for	'//최대 8개까지 제한
...
%>
			...
			<li>
			<% If InStr(link,"?") Then %>
				<a href="<%=link%>&gaparam=main_look_2<%=i+1%>">
			<% Else %>
				<a href="<%=link%>?gaparam=main_look_2<%=i+1%>">
			<% End If %>
