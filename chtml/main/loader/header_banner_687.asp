...
Dim sFolder, mainFile, i, CtrlDate, CtrlDate2,  existscnt, needcnt, idx, posname, existsidx
Dim image, image2, link, linktext, linktext2, linktext4, startdate, enddate
	...
	'//로고 CLASS
	dim sMainLogoClass: sMainLogoClass="proxyLogoV15"
	if Date>="2017-10-10" and Date<="2017-10-25" then
		'16주년
		sMainLogoClass = "proxyLogoV17"
	end if
...
If fileCont<>"" Then
	...
	For each tplNodes in cTmpl
		...
		if i>limitNumber then exit for	'//최대 6개까지 제한
...
'		If CtrlDate >= startdate AND CtrlDate <= enddate Then
		'2016-11-04 유태욱 추가
		If CDate(CtrlDate2) >= CDate(startdate) AND CDate(CtrlDate2) <= CDate(enddate) Then
...
%>
				<div class="slide<%=chkIIF(linktext4="wide"," wideBg","")%>" style="background-color:<%=linktext%>; background-image:url(<%=image%>)">
					<strong class="<%=sMainLogoClass & chkIIF(linktext2="wht"," wL","")%>"><a href="<%=wwwURL%>">10X10</a></strong>
					<% If InStr(link,"?") Then %>
...
'		If CtrlDate >= startdate AND instr(existsidx,idx)=0 Then
		'2016-11-04 유태욱 추가
		If CDate(CtrlDate2) >= CDate(startdate) AND instr(existsidx,idx)=0 Then
			if Not(image="" or isNull(image)) then
%>
