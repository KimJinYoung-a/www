...
Dim addtag_only : addtag_only = " <span class='labelV18 label-black'><b>단독</b></span>"
...
%>
			...
			<div class="rolling-item <%=chkiif(bantype="wide","bg-wide","")%> spc-weekendV19" style="background-image:url(<%=img%>);">
				<a href="<%=alink%>"  onclick="click_main_rolling('<%=intJ+1%>','<%=link%>','<%=maincopy%>','<%=subcopy%>','<%=etctag%>','<%=evt_code%>');">
					...
					<div class="text-img">
						...
						<p class="discount"><% If salePer<>"" Then %><b class="color-red"><%=salePer%>%</b><% End If %><% If saleCPer<>"" Then %><b class="color-green">쿠폰</b><% End If %></p>
					</div>
					<% End If %>
				</a>
			...
			<% If etctag="8" or etctag="10" Then %>
			<!-- 주말특가 A타입, B타입 구분 제거 -->
			<!-- div class="rolling-item <%=chkiif(bantype="wide","bg-wide","")%> spc-weekendV19 <%=chkiif(etctag="10","btype","")%>" style="background-image:url(<%=img%>);" -->
			<div class="rolling-item <%=chkiif(bantype="wide","bg-wide","")%> spc-weekendV21" style="background-image:url(<%=img%>);">
				<a href="<%=alink%>"  onclick="click_main_rolling('<%=intJ+1%>','<%=link%>','<%=maincopy%>','<%=subcopy%>','<%=etctag%>','<%=evt_code%>');">
					<div class="desc <%=chkiif(ftcolor=0,"color-black","color-white")%>">
						<div class="copy">
							<p><%=maincopy%>
					...
					<div class="desc <%=chkiif(ftcolor=0,"color-black","color-white")%>">
						<div class="copy">
							<p><%=maincopy%>
								...
								<% If salePer<>"" and (not isnull(salePer)) and salePer <> 0 Then %>
								<%=addtag(2,"~"&salePer&"%")%>
								<% End If %>
								<% If saleCPer<>"" and (not isnull(saleCPer)) Then %>
	...
	<script>
		...
		function click_main_rolling(index, link, evt_name, evt_subcopyK, evt_tag, evt_code){
            ...
            if(typeof qg !== "undefined"){
                ...
                qg("event", "event_clicked", appier_content_clicked);
            }
        }
	</script>
</div>
