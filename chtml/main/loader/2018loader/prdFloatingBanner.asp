...
<%
'#######################################################
' Discription : pc_prd_floating_banner // cache DB경유
' History : 2019-03-14 원승현 생성
'#######################################################
Dim intI
Dim sqlStr , rsMem , arrList, poscode
Dim gaParam : gaParam = "gaparam=item_popupbanner" '//GA 체크 변수

'//DB 시간 타이머 00시부터 00시 5분 사이에는 DB 캐쉬 1분에 한번 읽기
Dim cTime , dummyName
If timer > 10 And Cint(timer/60) < 6 Then
	cTime = 60*1
	dummyName = "MBPFB_"&Cint(timer/60)
Else
	cTime = 60*1
	dummyName = "MBPFB"
End If
...
set rsMem = getDBCacheSQL(dbget, rsget, "PRDFLOATINGBAN", sqlStr, cTime)
...
%>
	<% If linkurl <> "" And imageurl <> "" Then %>
        <% If Trim(request.Cookies("closeFloatingBanner"&idx&""))="" Then %>
            <style>
	    		.bnr-floationg {position:fixed; right:50%; bottom:150px; z-index:1001; width:180px; margin-right:-670px;}
				.bnr-floationg a {display:block;}
				.bnr-floationg button {display:block; margin:10px auto 0; background:transparent; outline:0;}
				.bnr-floationg img {vertical-align:top;}
			</style>
            <script>
            ...
            <div class="bnr-floationg" id="prdTempfloatingBanner">
                <a href="<%=linkurl%>"><img src="<%=staticImgUrl & "/main/" + db2Html(imageurl)%>" alt="<%=altname%>"></a>
                <button type="button" onclick="setCookieTempBanner('closeFloatingBanner<%=idx%>','Y',3);$('#prdTempfloatingBanner').hide();return false;"><img src="//webimage.10x10.co.kr/fixevent/event/2019/coupon/0121/btn_anymore.png" alt="오늘 그만보기"></a>
            </div>	
        <% End If %>
	<% End If %>
<% End If %>
