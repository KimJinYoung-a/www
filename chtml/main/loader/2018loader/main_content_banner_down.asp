...
<%
'#######################################################
' Discription : pc_main_content_banner_down // cache DB경유
' History : 2019-02-15 원승현 생성
'#######################################################
Dim intI
Dim sqlStr , rsMem , arrList, poscode
Dim gaParam : gaParam = "gaparam=main_content_banner_down_" '//GA 체크 변수

'//DB 시간 타이머 00시부터 00시 5분 사이에는 DB 캐쉬 1분에 한번 읽기
Dim cTime , dummyName
If timer > 10 And Cint(timer/60) < 6 Then
	cTime = 60*1
	dummyName = "MBCDBAN_"&Cint(timer/60)
Else
	cTime = 60*5
	dummyName = "MBCDBAN"
End If

'// 메인 컨텐츠 배너 하단(poscode)
poscode = "723"
...
set rsMem = getDBCacheSQL(dbget, rsget, "MAINCONDBAN", sqlStr, cTime)
