...
<%
'#######################################################
' Discription : pc_main_floating_banner // cache DB경유
' History : 2019-02-15 원승현 생성
'#######################################################
Dim intI
Dim sqlStr , rsMem , arrList, poscode
Dim gaParam : gaParam = "gaparam=main_floating_banner" '//GA 체크 변수

'//DB 시간 타이머 00시부터 00시 5분 사이에는 DB 캐쉬 1분에 한번 읽기
Dim cTime , dummyName
If timer > 10 And Cint(timer/60) < 6 Then
	cTime = 60*1
	dummyName = "MBFBAN_"&Cint(timer/60)
Else
	cTime = 60*1
	dummyName = "MBFBAN"
End If
...
If IsArray(arrList) Then
    ...
    End If
%>
    <% If imageurl<>"" And linkurl<>"" And imageurl2<>"" Then %>
        <script>
            $(function() {    
                $(window).scroll(function(){
                    var nowSt = $(this).scrollTop();
                    var lastSt = $('.md-pick').offset().top*.33;
                    if ( lastSt < nowSt ) {
        ...
        <div class="bnr-expand">
            <p><img src="<%=staticImgUrl & "/main/" + db2Html(imageurl)%>" alt="<%=altname%>"></p>
            <a href="<%=linkurl%>" onclick="fnAmplitudeEventAction('click_mainfloatingbanner','linkurl','<%=linkurl%>');" title=""><img src="<%=staticImgUrl & "/main2/" + db2Html(imageurl2)%>" alt=""></a>
        </div>
    <% End If %>
<% End If %>
<% on Error Goto 0 %>
