...
<%
'#######################################################
' Discription : pc_main_giftcard // cache DB경유
' History : 2018-03-06 이종화 생성
'#######################################################
Dim poscode , intI
Dim sqlStr , rsMem , arrList
Dim gaParam : gaParam = "?gaparam=main_giftcard" '//GA 체크 변수
...
If timer > 10 And Cint(timer/60) < 6 Then
	cTime = 60*1
	dummyName = "GFIMG_"&Cint(timer/60)
Else
	cTime = 60*5
	dummyName = "GFIMG"
End If
...
If IsArray(arrList) Then
	...
	For intI = 0 To ubound(arrlist,2)
		...
		alink = link & gaParam
%>
		<a href="<%=alink%>" onclick=AmpEventGiftCard();>
			<h2><b>GIFT CARD</b><small>센스있는 선물을 하고싶다면</small></h2>
			<div class="bnr"><img src="<%=img%>" alt="<%=altname%>" /></div>
			<div><script>function AmpEventGiftCard(jsonval){ AmplitudeEventSend('MainGiftCard', jsonval, 'eventProperties'); }</script></div>
		</a>
<%
	Next
End If
