...
<%
'#######################################################
' Discription : pc_main_bigevent_banner // cache DB경유
' History : 2019-02-14 원승현 생성
'#######################################################
Dim intI
Dim sqlStr , rsMem , arrList, poscode
Dim gaParam : gaParam = "gaparam=main_bigevent_" '//GA 체크 변수

'//DB 시간 타이머 00시부터 00시 5분 사이에는 DB 캐쉬 1분에 한번 읽기
Dim cTime , dummyName
If timer > 10 And Cint(timer/60) < 6 Then
	cTime = 60*1
	dummyName = "MBEBAN_"&Cint(timer/60)
Else
	cTime = 60*1
	dummyName = "MBEBAN"
End If
...
If IsArray(arrList) Then
    ...
    '// link3에 파라미터 있는지 체크
    If instr(linkurl3, "?")>0 Then
        linkurl3 = db2Html(linkurl3) & "&" &gaParam & "3"
    Else
        linkurl3 = db2Html(linkurl3) & "?" &gaParam & "3"
    End If
%>
    <div class="big-bnr">
        ...
        <% If linkurl2 <> "" And imageurl2 <> "" Then %>
            <a href="<%=linkurl2%>"><img src="<%=staticImgUrl & "/main2/" + db2Html(imageurl2)%>" alt="<%=altname2%>"></a>
        <% End If %>
        <% If linkurl3 <> "" And imageurl3 <> "" Then %>
            <a href="<%=linkurl3%>"><img src="<%=staticImgUrl & "/main3/" + db2Html(imageurl3)%>" alt="<%=altname3%>"></a>
        <% End If %>
    </div>
<% End If %>
<% on Error Goto 0 %>
