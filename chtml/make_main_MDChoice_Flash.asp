...
dim i, itemCnt
dim savePath, FileName, refip, refer
...
if (InStr(refer,"10x10.co.kr")<1) and (refip<>"110.93.128.113") and (refip<>"61.252.133.70") then
...
if ocontents.FResultCount>0 then
		...
		for i=0 to ocontents.FResultCount-1
			Set objXMLv = objXML.createElement("item")
				...
				objXMLv.childNodes(5).appendChild(objXML.createCDATASection("startdate"))
				objXMLv.childNodes(5).childNodes(0).text = db2Html(ocontents.FItemList(i).Fstartdate)

				objXMLv.appendChild(objXML.createElement("enddate"))
				objXMLv.childNodes(6).appendChild(objXML.createCDATASection("enddate"))
				objXMLv.childNodes(6).childNodes(0).text = db2Html(ocontents.FItemList(i).Fenddate)

				objXMLv.appendChild(objXML.createElement("idx"))
				objXMLv.childNodes(7).text = ocontents.FItemList(i).Fidx
