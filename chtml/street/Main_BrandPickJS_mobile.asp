    ...
    If CInt(vImageCount) >= CInt(vTotalCount) Then
		...
		End If
			...
			if instr(fnGetList(0,i-1),"http://")>0 then
				BufStr = BufStr + VarName + "+=" + DoubleQuat + "	<a href='/street/street_brand.asp?makerid="& fnGetList(7,i-1) &"'><img src='" & db2Html(fnGetList(0,i-1)) &"' alt='브랜드Pick"&i&"' /></a>" + DoubleQuat + VbCrlf
			else
				BufStr = BufStr + VarName + "+=" + DoubleQuat + "	<a href='/street/street_brand.asp?makerid="& fnGetList(7,i-1) &"'><img src='" & staticImgUrl & "/brandstreet/main/" & db2Html(fnGetList(0,i-1)) &"' alt='브랜드Pick"&i&"' /></a>" + DoubleQuat + VbCrlf
			end if
			BufStr = BufStr + VarName + "+=" + DoubleQuat + "</div>" + DoubleQuat + VbCrlf
		Next
			BufStr = BufStr + VarName + "+=" + DoubleQuat + "</div>" + DoubleQuat + VbCrlf
...
alert('Mobile OK');
location.href="<%=wwwUrl%>/chtml/street/Main_BrandPickJS_app.asp?idx=<%=idx%>&imagecount=<%=imagecount%>";
</script>
