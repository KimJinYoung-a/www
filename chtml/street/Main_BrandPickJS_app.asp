    ...
    If CInt(vImageCount) >= CInt(vTotalCount) Then
		...
		End If
			...
			if instr(fnGetList(0,i-1),"http://")>0 then
				BufStr = BufStr + VarName + "+=" + DoubleQuat + "	<a href='javascript:fnAPPpopupBrand('"& fnGetList(7,i-1) &"');return false;'><img src='" & db2Html(fnGetList(0,i-1)) &"' alt='브랜드Pick"&i&"' /></a>" + DoubleQuat + VbCrlf
			else
