...
'// 변수 선언
	...
	Dim image, link, posname, startdate, enddate, idx, vDisp, evtname, evtcode, evtsubcopyK, etcitemimg, ab, tmpEvtName, evtSaleVal, etcitemid, etcimg, basicimg, basicimg600
	vDisp = request("disp")
	ab = request("ab")
...
'				If ab="002_b_5" Then
					...
					If Trim(image) <> "" Then
						Response.Write "	<a href='/event/eventmain.asp?eventid="& evtcode & CateMain_GaParam(vDisp,"mainroll",i+1) &"' onclick=fnAmplitudeEventMultiPropertiesAction('click_category_main_event','indexnumber|eventcode','"& i+1 & "|" & evtcode &"');><img src='"&image&"' alt='"&evtname&"' /></a> " & vbCrLf
					Else
						Response.Write "	<a href='/event/eventmain.asp?eventid="& evtcode & CateMain_GaParam(vDisp,"mainroll",i+1) &"' onclick=fnAmplitudeEventMultiPropertiesAction('click_category_main_event','indexnumber|eventcode','"& i+1 & "|" & evtcode &"');><img src='"&etcimg&"' alt='"&evtname&"' /></a> " & vbCrLf
					End If
