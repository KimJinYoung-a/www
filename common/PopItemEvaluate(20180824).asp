				...
				<div class="popReviewV15">
					...
					<div class="review pdtReviewV15">
						...
						<input type="hidden" name="sortMethod" value="<%=sortMethod%>">
							...
							<ul class="tabMenuV15">
								<li><a href="" <%=chkiif(sortMethod="" or sortMethod="ne" or sortMethod="be","class=""on""","")%> onclick="chgEvalVal('a'); return false;" id="evalall">전체 (<strong><%= formatNumber(oEvalCnt.FEvalItem.FEvalCnt,0) %></strong>)</a></li>
								<li><a href="" <%=chkiif(sortMethod="ph","class=""on""","")%> onclick="chgEvalVal('p'); return false;" id="evalph">포토 (<strong><%= formatNumber(oEvalCnt.FEvalItem.FEvalcnt_photo,0) %></strong>)</a></li>
								<% if oEvalCnt.FEvalItem.FEvalcnt_tester>0 then %>
								<li><a href="" <%=chkiif(sortMethod="tt","class=""on""","")%> onclick="chgEvalVal('t'); return false;" id="evaltester">테스터 후기 (<strong><%= formatNumber(oEvalCnt.FEvalItem.FEvalcnt_tester,0) %></strong>)</a></li>
								<% end if %>
							</ul>
							...
							<% if Eval_excludeyn="N" then %>
								...
								<select name="sortMtd" title="상품 후기 정렬 옵션" class="optSelect2" onchange="TnMovePage(1)">
