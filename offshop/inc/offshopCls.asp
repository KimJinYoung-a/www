...
Class COffShopQNA
	...
	public Function fnGetShopQNA()
		...
		If Fuserid<>"" Then
		strSqlCnt = strSqlCnt & "	and userid='" & Fuserid &"'" & vbCrLf
		End If
		If FRectSearchText<>"" Then
			strSqlCnt = strSqlCnt & " and " & FRectSearchDiv & " like '%" & Cstr(FRectSearchText) &"%'" & vbCrLf
		End If
		rsget.Open strSqlCnt,dbget 
		IF not rsget.Eof THEN
			FTotCnt = rsget(0)
		...
		IF 	FTotCnt > 0 THEN
			intDelCnt =  (FCPage - 1) * FPSize 			
			strSql = " SELECT TOP "&FPSize&" q.idx, q.shopid, q.userid, q.title, q.regdate, q.replyuser, q.replydate, q.itemid, q.contents, q.replycontents, i.icon1image, i.brandname, i.itemname, q.username " & vbCrLf
			strSql = strSql & " FROM [db_shop].[dbo].tbl_offshop_qna q left join [db_item].[dbo].tbl_item i on i.itemid=q.itemid" & vbCrLf
			strSql = strSql & "  WHERE q.isusing='Y'  and q.shopid='"&FShopId&"'" & vbCrLf
			strSql = strSql & "	and q.idx not in ( SELECT TOP "&intDelCnt&" idx FROM [db_shop].[dbo].tbl_offshop_qna " & vbCrLf
			strSql = strSql & "				WHERE isusing='Y' and shopid='"&FShopId&"' order by idx DESC)" & vbCrLf
			If Fuserid<>"" Then
			strSql = strSql & "	and q.userid='" & Fuserid &"'" & vbCrLf
			End If
			If FRectSearchText<>"" Then
				strSql = strSql & " and q." & FRectSearchDiv & " like '%" & Cstr(FRectSearchText) &"%'" & vbCrLf
			End If
			strSql = strSql & "	order by q.idx DESC "
