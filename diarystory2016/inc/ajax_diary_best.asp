...
<%
'####################################################
' Description : 다이어리 스토리2016 seller(best),wish,review,event
...
%>
	<!-- SELLER -->
	<div id="seller" class="diaryList type01">
		<ul>
		<%
...
<% elseif gubunreturn="f" then %>
	<!-- WISH -->
	<div id="wish" class="diaryList type01">
		<ul>
		<%
		If awardlist.FResultCount > 0 Then
			For i = 0 To awardlist.FResultCount - 1
...
<% elseif gubunreturn="r" then %>
	<!-- REVIEW -->
	<div id="review" class="diaryList type02">
		<ul>
		<%
		If awardlist.FResultCount > 0 Then
			For i = 0 To awardlist.FResultCount - 1
				...
				<div class="pdtBox">
					...
					<div class="pdtInfo">
						...
						<div class="reviewBoxV15">
							<p class="starView"><img src="//fiximage.10x10.co.kr/web2019/common/ico_review_star_0<%= tmpreview(1) %>.png" alt="별<%= tmpreview(1) %>개" /></p>
							<p class="tPad10"><% = chrbyte(tmpreview(0),80,"Y") %></p>
...
<% elseif gubunreturn="e" then %>
	...
	%>
	<!-- EVENT -->
	<div id="event" class="diaryList type02">
		<ul>
					...
					<div class="pdtBox">
						<a href="/event/eventmain.asp?eventid=<%=odibest.FItemList(i).fevt_code %>">
							...
							<div class="pdtInfo">
								...
								<p class="evtExp"><%=odibest.FItemList(i).FEvt_subcopyK %>...</p>
		...
		<a href="/diarystory2016/event.asp" class="btnMore">다이어리 이벤트 더보기</a>
	</div>
	<!--// EVENT -->
<% end if %>
