...
<%
'####################################################
' Description : 다이어리 스토리2016 검색결과페이지
...
<script type="text/javascript">
	...
	function goSearchDiary()
	{
		...
		if ($("input:checkbox[id='optS05']").is(":checked")==true){
			document.frm_search1.limited.value = "o";
		}
		else{
			document.frm_search1.limited.value = "x";
		}
	...
	$(function(){		
		...
		$(".etc ul li a").mouseover(function(){
			$(this).find("span").fadeIn();
		});
		$(".etc ul li a").mouseleave(function(){
	...
	<div class="container diarystory2016">
		<div id="contentWrap">
			...
			<div class="diaryContent diarySearchResult">
							...
							<dl class="optionType02">
								...
								<dd>
									<dl class="dateType">
										...
										<dd>
											<ul class="optionList">
												<li><input type="checkbox" class="check" id="optCt01" name="contents" value="'only 2016'" <%= getchecked(arrcontents,"'only 2016'") %>/> <label for="optCt01">Only 2016</label></li>
												...
												<li class="sEdition"><input type="checkbox" class="check" id="optS05" name="limited" value="<%=limited%>" <% if trim(limited)="o" then %>checked<% end if %>  /> <label for="optS05"><strong>10X10 SPECIAL EDITION</strong></label></li>												
											</ul>
										</dd>
									</dl>
									<dl class="layout">
				...
				<div class="diaryCtgy">
					<div class="array">
						<p class="total">total <strong><%=PrdBrandList.FTotalCount%></strong></p>
						<div class="option">
							<select name="select" onchange="fnSearch(this.form.srm,this.value);" title="다이어리 상품 정렬 방식 선택" class="optSelect">
