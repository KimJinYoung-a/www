<%
	...
	tmpSQL = "Execute [db_item].[dbo].usp_WWW_Item_EventList_Get @vItemid=" & itemid &", @device='W'"
...
'	rsget.close
	...
	'// http -> https
    Function getThisEventSslLink(page)
        If LEFT(page,5) = "https" Then
            getThisEventSslLink = page
        ElseIf LEFT(page,4) = "http" Then
            getThisEventSslLink = "https" & RIGHT(page, LEN(page)-4)
        ELse
            getThisEventSslLink = SSLUrl & page
        End If
    End Function
		...
		FOR i=0 to ubound(arrEvt,2)
			...
			SELECT CASE cStr(arrEvt(0,i))
				...
				case "1" '/쇼핑 찬스
					IF arrEvt(6,i)="I" and arrEvt(7,i)<>"" Then
						strLink = "<a href="""& getThisEventSslLink(arrEvt(7,i) & chkIIF(instr(arrEvt(7,i),"?")>0,"&","?") & strEvtRc) & """ onclick=""fnAmplitudeEventMultiPropertiesAction('click_planevents_in_product','idx|evtcode','"& i+1 &"|"& arrEvt(4,i) &"')"">"
					Else
						strLink = "<a href=""" & SSLUrl & "/event/eventmain.asp?eventid=" & arrEvt(4,i) & "&"&strEvtRc & """ onclick=""fnAmplitudeEventMultiPropertiesAction('click_planevents_in_product','idx|evtcode','"& i+1 &"|"& arrEvt(4,i) &"')"">"
					End If
					strLinkName = db2html(arrEvt(2,i))
				case "7" '/위클리 코디네이터 (x)
					strLink = "<a href=""" & SSLUrl & "/guidebook/weekly_coordinator.asp?eventid=" & db2html(arrEvt(4,i)) & "&"&strEvtRc & """ onclick=""fnAmplitudeEventMultiPropertiesAction('click_planevents_in_product','idx|evtcode','"& i+1 &"|"& arrEvt(4,i) &"')"">"
					strLinkName = db2html(arrEvt(2,i))
				case "11" '/디자인 핑거스 (x)
					strLink = "<a href=""" & getThisEventSslLink(db2html(arrEvt(1,i)) & chkIIF(instr(arrEvt(1,i),"?")>0,"&","?") & strEvtRc) & """>"
					strLinkName = db2html(arrEvt(2,i)) & " " & CStr(arrEvt(3,i)) &"회차<" & db2html(arrEvt(10,i)) & ">"
				case "12" '/전체이벤트
					IF arrEvt(6,i)="I" and arrEvt(7,i)<>"" Then
						strLink = "<a href="""& getThisEventSslLink(arrEvt(7,i) & chkIIF(instr(arrEvt(7,i),"?")>0,"&","?") & strEvtRc) & """>"
					Else
						strLink = "<a href=""" & SSLUrl & "/event/eventmain.asp?eventid=" & arrEvt(4,i) & "&"&strEvtRc & """ onclick=""fnAmplitudeEventMultiPropertiesAction('click_planevents_in_product','idx|evtcode','"& i+1 &"|"& arrEvt(4,i) &"')"">"
					End if
					strLinkName = db2html(arrEvt(2,i))
				...
				case "17" '/다이어리 이벤트 (x)
					IF arrEvt(6,i)="I" and arrEvt(7,i)<>"" Then
						strLink = "<a href="""& getThisEventSslLink(arrEvt(7,i)) &""">"
						strLinkName = db2html(arrEvt(2,i))
					ELSE
						strLink = "<a href=""" & SSLUrl & "/diary2010/diary_event_view.asp?eventid="& arrEvt(4,i) &""">"
						strLinkName = db2html(arrEvt(2,i))
					END IF
				case "23" '/테스터이벤트
					strLink = "<a href=""" & SSLUrl & "/event/eventmain.asp?eventid=" & arrEvt(4,i) & "&"&strEvtRc & """>"
					strLinkName = db2html(arrEvt(2,i))
				case "29" '/Hey Something Project
					strLink = "<a href=""" & SSLUrl & "/HSProject/?eventid=" & arrEvt(4,i) & "&"&strEvtRc & """>"
					strLinkName = db2html(arrEvt(2,i))
				case "31" '/브랜드위크
					IF arrEvt(6,i)="I" and arrEvt(7,i)<>"" Then
						strLink = "<a href="""& getThisEventSslLink(arrEvt(7,i) & chkIIF(instr(arrEvt(7,i),"?")>0,"&","?") & strEvtRc) &""">"
			...
			If arrEvt(33,i)="80" Then
				If arrEvt(20,i)="5" Then
					...
					'사은품 소진 체크
					if arrEvt(36,i)>0 and arrEvt(10,i)<1 then
						itEvtBanner=itEvtBanner+"		<div class='banner_end diary2023'>"
						itEvtBanner=itEvtBanner+"			<p>이벤트 종료</p>"
						itEvtBanner=itEvtBanner+"			<p>아쉽게도 모든 재고가 소진되었어요 :)</p>"
					...
					If arrEvt(23,i)="1" Then
						...
						itEvtBanner=itEvtBanner+FnChangeTextImage(arrEvt(23,i),CLng((oItem.Prd.FOrgPrice-oItem.Prd.FSellCash)/oItem.Prd.FOrgPrice*100))
					...
					if year(arrEvt(12,i))>year(arrEvt(11,i)) then
					itEvtBanner=itEvtBanner+"	<span class='date'>("+ FormatDate(arrEvt(11,i),"0000.00.00") + "~" + FormatDate(arrEvt(12,i),"0000.00.00") + ")</span>"
					else
					...
					Else
						...
						If arrEvt(26,i) <> "" Then
							If arrEvt(28,i) <> "" Then
								...
								itEvtBanner=itEvtBanner+arrEvt(26,i)
							End If
						End If
						If arrEvt(28,i) <> "" Then
...
$(function() {
	/*$('.evtIssuV17a ul li').each(function(){
		var tagWidth = $(this).find('.evtTit dfn').width()+10;
	...
	$('.evtIssuV17a ul li').each(function(){
		var tagWidth = $(this).find('dfn').outerWidth();
		$(this).find('.evtTit').css('padding-right',tagWidth+'px');
		var txtWidth = $(this).find('.evtTit').outerWidth();
		if (txtWidth > 355) {
			$(this).find('.evtTit > strong').css('width',345-tagWidth+'px');
		}
	});
