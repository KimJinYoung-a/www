...
Dim ab : ab = requestCheckVar(request("ab"),18)
...
if Not IsValidStyleCD(styleCD) then
    styleCD = ""
end if
...
'디자인 문구 제외한 나머지 카테고리 인기순 20190412
If SortMet="" Then
	'//디자인 문구 카테고리 인기순 수정 2021.10.01 정태훈
	'If dispCate="101" Then
	'	SortMet = "ne"
	'Else
		SortMet = "be"
	'End If
...
dim vAmplitudeSearchFlag, vAmplitudeSortMet, vAmplitudeSellScope, vAmplitudeCategoryDepth
dim vAmplitudeMinPrice, vAmplitudeMaxPrice, vAmplitudeStyleCd, vAmplitudeColorCD, vAmplitudeDeliType

'category_name : CategoryNameUseLeftMenuDB(Left(dispCate, 3))
'categoryCode : dispcate
'category_depth : vAmplitudeCategoryDepth
...
<script type="text/javascript" src="/lib/js/searchFilter_categorymain.js?v=1.1"></script>
<script type="text/javascript" src="/lib/js/jquery.numspinner.min.js"></script>
<script>
	// fnAmplitudeEventMultiPropertiesAction("click_category_productlist_filter"
	// ,"category_code|category_depth|category_name|sort|style|color|delivery|min_price|max_price|list_type|soldout|list_style","");
	<%'일단 사용 안하는 이벤트 20180726 //dispcate,vAmplitudeCategoryDepth,CategoryNameUseLeftMenuDB(Left(dispCate, 3)),vAmplitudeSortMet,vAmplitudeStyleCd,vAmplitudeColorCD,vAmplitudeDeliType,vAmplitudeMinPrice,vAmplitudeMaxPrice,vAmplitudeSearchFlag,vAmplitudeSellScope,icoSize%>
	fnAmplitudeEventMultiPropertiesAction('view_category_main','categorycode|categoryname|depth','<%=dispcate%>|<%=CategoryNameUseLeftMenuDB(Left(dispCate, 3))%>|<%=vAmplitudeCategoryDepth%>');
	<% if CurrPage > 1 then %>
	fnAmplitudeEventMultiPropertiesAction('click_category_main_paging','paging_index|sort','<%=CurrPage%>|<%=vAmplitudeSortMet%>');
	<%end if%>
$(function() {
	//LNB Control
	...
	// select design
	$(".evtSelectV15a p").click(function(){
		if($(".evtSelectV15a div").is(":hidden")){
			$(this).parent().children('div').show("slide", { direction: "up" }, 300);
			$(this).addClass("over");
		}else{
			$(this).parent().children('div').hide("slide", { direction: "up" }, 200);
		};
	});
	$(".evtSelectV15a div li").click(function(){
		var evtName = $(this).text();
		$(".evtSelectV15a p").removeClass("over");
		$(".evtSelectV15a div li").removeClass("on");
		$(this).addClass("on");
		$(this).parent().parent().parent().children('p').children('span').text(evtName);
		$(this).parent().parent().hide("slide", { direction: "up" }, 200);
	});
	$(".evtSelectV15a div").mouseleave(function(){
		$(this).hide();
		$(".evtSelectV15a p").removeClass("over");
	});
...
function amplitudeChangeSortSend(a)
{
	...
	fnAmplitudeEventMultiPropertiesAction('click_category_main_productlist_sort'
	...
	,sendsortvalue+'|<%=vAmplitudeSortMet%>|<%=dispcate%>');
				...
				<!-- #Include virtual="/search/inc_searchFilter_categorymain.asp" -->
...
%>
				<div class="section">
					<form name="sFrm" id="listSFrm" method="get" action="/shopping/category_main.asp" style="margin:0px;">
					...
					<input type="hidden" name="srm" id="srm" value="<%= SortMet %>">
						...
						<% If oDoc.FItemList(icol).FItemDiv="21" Then %>
							<div class="pdtBox">
								...
								<div class="pdtPhoto">
									...
									<% end if %>
									<%'<!-- 20190222 성인인증 안내 -->%>
									<a href="/deal/deal.asp?itemid=<%= oDoc.FItemList(icol).FItemID %>&disp=<%=getArrayDispCate(dispCate,oDoc.FItemList(icol).FarrCateCd)%><%=logparam%><%=CateMain_GaParam(dispCate,"item",icol)%>">
								...
								<div class="pdtInfo">
									<p class="pdtBrand tPad20"><a href="/street/street_brand.asp?makerid=<%= oDoc.FItemList(icol).FMakerid %>" onclick="fnAmplitudeEventMultiPropertiesAction('click_category_main_product_brand', 'item_index|sort|category_code|category_depth|itemid|category_name|brand_name|list_style','<%=icol+1%>|<%=vAmplitudeSortMet%>|<%=dispcate%>|<%=vAmplitudeCategoryDepth%>|<%=oDoc.FItemList(icol).FItemID%>|<%=fnCateCodeToCategory1DepthName(Left(Trim(oDoc.FItemList(icol).FCateCode),3))%>|<%=replace(Replace(oDoc.FItemList(icol).FBrandName," ",""),"'","`")%>|<%=icoSize%>')";><% = oDoc.FItemList(icol).FBrandName %></a></p>
									<p class="pdtName tPad07"><a href="/deal/deal.asp?itemid=<%= oDoc.FItemList(icol).FItemID %>&disp=<%=getArrayDispCate(dispCate,oDoc.FItemList(icol).FarrCateCd)%><%=logparam%><%=CateMain_GaParam(dispCate,"item",icol)%>"><% = oDoc.FItemList(icol).FItemName %></a></p>
									<% IF oDoc.FItemList(icol).FItemOptionCnt="" Or oDoc.FItemList(icol).FItemOptionCnt="0" then %>
									<p class="pdtPrice"><span class="finalP"><%=FormatNumber(oDoc.FItemList(icol).getRealPrice,0)%>원</span><% If oDoc.FItemList(icol).FtenOnlyYn="Y" Then %>~<% End If %></p>
									<% Else %>
									<p class="pdtPrice"><span class="finalP"><%=FormatNumber(oDoc.FItemList(icol).getRealPrice,0)%>원</span><% If oDoc.FItemList(icol).FtenOnlyYn="Y" Then %>~<% End If %> <strong class="cRd0V15">[<% If oDoc.FItemList(icol).FLimityn="Y" Then %>~<% End If %><%=oDoc.FItemList(icol).FItemOptionCnt%>%]</strong></p>
									<% End If %>
									...
									<% Else %>
										...
										<% IF oDoc.FItemList(icol).FLimityn="Y" Then %><img src="http://fiximage.10x10.co.kr/web2013/shopping/tag_sale.gif" alt="SALE" /> <% end if %>
						...
						<% Else %>
							<div class="pdtBox">
								...
								<div class="pdtPhoto">
									...
									<% end if %>
									<%'<!-- 20190222 성인인증 안내 -->%>
									<a href="/shopping/category_prd.asp?itemid=<%= oDoc.FItemList(icol).FItemID %>&disp=<%=getArrayDispCate(dispCate,oDoc.FItemList(icol).FarrCateCd)%><%=logparam%><%=CateMain_GaParam(dispCate,"item",icol)%>" onclick="window.event.cancelBubble=true; fnAmplitudeEventMultiPropertiesAction('click_category_main_product', 'item_index|sort|category_code|category_depth|itemid|category_name|brand_name|list_style','<%=icol+1%>|<%=vAmplitudeSortMet%>|<%=dispcate%>|<%=vAmplitudeCategoryDepth%>|<%=oDoc.FItemList(icol).FItemID%>|<%=fnCateCodeToCategory1DepthName(Left(Trim(oDoc.FItemList(icol).FCateCode),3))%>|<%=replace(Replace(oDoc.FItemList(icol).FBrandName," ",""),"'","`")%>|<%=icoSize%>');">
										<span class="soldOutMask"></span>
										<% if icoSize="M" then %>
										<img src="<%=getThumbImgFromURL(oDoc.FItemList(icol).FImageBasic,imgSz,imgSz,"true","false")%>" alt="<%=Replace(oDoc.FItemList(icol).FItemName,"""","")%>" />
										<% else %>
								...
								<ul class="pdtActionV15">
									<li class="largeView"><a href="" onclick="fnAmplitudeEventMultiPropertiesAction('click_category_main_product_info','type','quick'); ZoomItemInfo('<%=oDoc.FItemList(icol).FItemid %>'); return false;"><img src="http://fiximage.10x10.co.kr/web2015/common/btn_quick.png" alt="QUICK" /></a></li>
									<li class="postView"><a href="" onclick="<%=chkIIF(oDoc.FItemList(icol).FEvalCnt>0,"popEvaluate('" & oDoc.FItemList(icol).FItemid & "');","")%> fnAmplitudeEventMultiPropertiesAction('click_category_main_product_info','type','review'); return false;"><span><%=oDoc.FItemList(icol).FEvalCnt%></span></a></li>
									<li class="wishView"><a href="" onclick="fnAmplitudeEventMultiPropertiesAction('click_category_main_product_info','type','wish'); TnAddFavorite('<%=oDoc.FItemList(icol).FItemid %>'); return false;"><span><%=oDoc.FItemList(icol).FfavCount%></span></a></li>
								</ul>
							</div>
						<% End If %>
						</li>
						...
						<li class="<%=classStr%>" <%=chkIIF(adultChkFlag, "onclick=""confirmAdultAuth('"& linkUrl &"');""","")%>>
							<div class="pdtBox">
								...
								<div class="pdtPhoto">
									...
									<% end if %>
									<%'성인상품 블라인드%>
									<span class="soldOutMask"></span><a href="/shopping/category_prd.asp?itemid=<%=oEval.FItemList(lp).Fitemid%><%=logparam%><%=CateMain_GaParam(dispCate,"item",lp)%>" onclick="window.event.cancelBubble=true; fnAmplitudeEventMultiPropertiesAction('click_category_main_product', 'item_index|sort|category_code|category_depth|itemid|category_name|brand_name|list_style','<%=icol+1%>|<%=vAmplitudeSortMet%>|<%=dispcate%>|<%=vAmplitudeCategoryDepth%>|<%=oDoc.FItemList(icol).FItemID%>|<%=fnCateCodeToCategory1DepthName(Left(Trim(oDoc.FItemList(icol).FCateCode),3))%>|<%=replace(Replace(oDoc.FItemList(icol).FBrandName," ",""),"'","`")%>|<%=icoSize%>');"><img src="<%=oEval.FItemList(lp).FIcon1Image%>" width="200px" height="200px" alt="<%=replace(oEval.FItemList(lp).Fitemname,"""","")%>" /></a>
								</div>
								<div class="pdtInfo ftRt">
									<p class="pdtBrand"><a href="/street/street_brand.asp?makerid=<%=oEval.FItemList(lp).FMakerId%>" onclick="fnAmplitudeEventMultiPropertiesAction('click_category_main_product_brand', 'item_index|sort|category_code|category_depth|itemid|category_name|brand_name|list_style','<%=icol+1%>|<%=vAmplitudeSortMet%>|<%=dispcate%>|<%=vAmplitudeCategoryDepth%>|<%=oDoc.FItemList(icol).FItemID%>|<%=fnCateCodeToCategory1DepthName(Left(Trim(oDoc.FItemList(icol).FCateCode),3))%>|<%=replace(Replace(oDoc.FItemList(icol).FBrandName," ",""),"'","`")%>|<%=icoSize%>')";><%=oEval.FItemList(lp).Fbrandname%></a></p>
								...
								<ul class="pdtActionV15">
									<li class="largeView"><a href="" onclick="fnAmplitudeEventMultiPropertiesAction('click_category_main_product_info','type','quick'); ZoomItemInfo('<%=oEval.FItemList(lp).FItemid %>'); return false;"><img src="http://fiximage.10x10.co.kr/web2015/common/btn_quick.png" alt="QUICK" /></a></li>
									<li class="postView"><a href="" <%=chkIIF(oEval.FItemList(lp).FEvalCnt>0,"onclick=""popEvaluate('" & oEval.FItemList(lp).FItemid & "'); fnAmplitudeEventMultiPropertiesAction('click_category_main_product_info','type','review'); return false;""","")%>><span><%=oEval.FItemList(lp).FEvalcnt%></span></a></li>
									<li class="wishView"><a href="" onclick="fnAmplitudeEventMultiPropertiesAction('click_category_main_product_info','type','wish'); TnAddFavorite('<%=oEval.FItemList(lp).FItemid %>'); return false;"><span><%=oEval.FItemList(lp).FfavCount%></span></a></li>
								</ul>
							</div>
...
%>
			...
			<form name="sFrm" id="listSFrm" method="get" action="/shopping/category_main.asp">
			<input type="hidden" name="rect" value="<%= SearchText %>">
			<input type="hidden" name="prvtxt" value="<%= PrevSearchText %>">
			<input type="hidden" name="rstxt" value="<%= ReSearchText %>">
			<input type="hidden" name="extxt" value="<%= ExceptText %>">
...
<%
	If oDoc.FResultCount > 0 Then
		Dim CriteoCategoryItemListsCategoryMain
		'// 크리테오 전송용 상품코드
		CriteoCategoryItemListsCategoryMain = ""

		For icol = 0 To oDoc.FResultCount - 1
			If icol < 3 Then
				CriteoCategoryItemListsCategoryMain = CriteoCategoryItemListsCategoryMain & "'"&oDoc.FItemList(icol).FItemID&"',"
			End If
		Next
		If Trim(CriteoCategoryItemListsCategoryMain) <> "" Then
			CriteoCategoryItemListsCategoryMain = Left(CriteoCategoryItemListsCategoryMain, Len(CriteoCategoryItemListsCategoryMain)-1)
		End If
		'//크리테오에 보낼 md5 유저 이메일값
		Dim CriteoUserMailMD5InCategoryMain
		If Trim(session("ssnuseremail")) <> "" Then
			CriteoUserMailMD5InCategoryMain = MD5(Trim(session("ssnuseremail")))
		Else
			CriteoUserMailMD5InCategoryMain = ""
		End If
%>
		...
		window.criteo_q.push(
			...
			{ event: "setEmail", email: "<%=CriteoUserMailMD5InCategoryMain%>" },
			{ event: "setSiteType", type: deviceType},
			{ event: "viewList", item: [<%=CriteoCategoryItemListsCategoryMain%>] }
		);
		</script>
<%
	End If
