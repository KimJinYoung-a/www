...
function jsGoEventPage(pg) {
	$.ajax({
		url: "act_happyTogether_New.asp?itemid=<%=itemid%>&disp=<%=catecode%>&rtype=<%=rtype%>&cpg="+pg,
		cache: false,
		async: false,
	...
	<div class="itemContainerV17a">
		<div class="itemContV15">
			...
			<% if lp>4 then Exit For %>
				<li>
					<p class="pdtPhoto">
						<a href="/shopping/category_prd.asp?itemid=<%= oHTBCItem.FItemList(lp).Fitemid %>&rc=item_happy_<%=lp+1%>&disptype=<%=CHKIIF(rtype="1","n","g")%>" onclick="fnAmplitudeEventMultiPropertiesAction('click_happytogether_in_product','idx|itemid','<%= lp + 1 %>|<%= oHTBCItem.FItemList(lp).Fitemid %>')">
							<img src="<%=getThumbImgFromURL(oHTBCItem.FItemList(lp).FIcon1Image,150,150,"true","false") %>" width="150px" height="150px" alt="<%=oHTBCItem.FItemList(lp).FItemName%>" />
						</a>
					</p>
					...
					<p class="pdtName tPad05">
						<a href="/shopping/category_prd.asp?itemid=<%= oHTBCItem.FItemList(lp).Fitemid %>&rc=item_happy_<%=lp+1%>&disptype=<%=CHKIIF(rtype="1","n","g")%>" onclick="fnAmplitudeEventMultiPropertiesAction('click_happytogether_in_product','idx|itemid','<%= lp + 1 %>|<%= oHTBCItem.FItemList(lp).Fitemid %>')">
							<%=chrbyte(oHTBCItem.FItemList(lp).FItemName, 12, "Y")%>
						</a>
					</p>
