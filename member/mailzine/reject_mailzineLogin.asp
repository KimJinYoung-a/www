...
function fnRejectLogin() {
    location.replace("/my10x10/userinfo/membermodify.asp?rdsite=<%=rdsite%>&utm_source=<%=utm_source%>&utm_medium=<%=utm_medium%>&utm_campaign=<%=utm_campaign%>")
    return;
}
		...
		<div id="contentWrap">
			<div id="lyrForm" class="mailReject">
				<div class="case04">
					...
					<p class="main_copy">로그인 후 수신 거부가 가능해요.</p>
					<p class="sub_copy">로그인 후 <span>마이 텐바이텐 <i><img src="http://fiximage.10x10.co.kr/web2022/common/arrow.png" alt=""></i> 개인정보수정</span>에서<br/>이메일 수신 여부를 설정해 주세요 🙂 </p>
					<div class="btnArea">
						<a href="#" onclick="fnRejectLogin(); return false;" class="btn_ten">로그인하러 가기</a>
					</div>
				</div>
			</div>
		</div>
