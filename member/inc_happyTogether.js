...
function getRecoListCheck(vCHkHT, vRuid)
{
	...
	if (vCHkHT=="N")
	{
		$.getJSON("https://api.recopick.com/v1/recommendations/most/86/"+vRuid+"?limit=10&channel=recopick_member&type=view2&callback=?",
...
function getRecoListCheckView(vCHkHT, vRuid)
{
	...
	if (vCHkHT=="N")
	{
		$.getJSON("https://api.recopick.com/v1/recommendations/most/86/"+vRuid+"?limit=10&channel=recopick_member&type=view&callback=?",
			function(data, status)
			{
				if (status=="success")
				{
...
function FnGoProdItem(vi,oi,mt,dv,uid,lnk,chn){
	...
	//} else {
		if(lnk!="") {
			...
			location.href = "https://api.recopick.com/1/banner/86/pick?source=" + oi + "&pick=" + vi + "&method=" + mt + "&channel=recopick_member&reco_type=item-item";
