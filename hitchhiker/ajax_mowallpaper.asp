					...
					<ul>
						...
						<% for j = 0 to olink.fresultcount-1 %>
							<% if len(trim(olink.FItemList(j).FContentslink)) > 4 then %>
								<li><a href="<%= olink.FItemList(j).FContentslink %>"><strong><%= olink.FItemList(j).FContentsSize %></strong> <span><%=olink.FItemList(j).FDevicename%></span></a></li>
							<% else %>
								<li><a href="javascript:fileDownload('<%= olink.FItemList(j).FContentslink %>');"><strong><%= olink.FItemList(j).FContentsSize %></strong> <span><%=olink.FItemList(j).FDevicename%></span></a></li>
							<% end if %>
						<% next %>
						<% end if %>
					</ul>
