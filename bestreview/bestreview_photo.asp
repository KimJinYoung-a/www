...
<%
	strPageTitle = "텐바이텐 10X10 : 베스트 리뷰 - 포토후기"		'페이지 타이틀 (필수)
%>
...
<%
	...
	oEval.FPageSize = 8
	oEval.FRectMode = "photo"
	oEval.FRegdateS = Left(dateAdd("d",-31,now()),10) 	''검색 느림 날짜 조건 추가 /eastone /1달
			...
			<div class="hotHgroupV19 bg-orange">
				...
				<div class="grpSubWrapV19">
					<ul>
						...
						<li class="nav2 on"><a href="/bestreview/bestreview_photo.asp?disp=<%=catecode%>">포토후기</a></li>
				...
				<div class="hotArticleV15">
					...
					<% If oEval.FTotalCount > 0 Then %>
						<div class="pdtWrap reviewListV15 photoReviewWrap">
							...
							For lp=0 To (oEval.FResultCount-1)
							%>
								<li <% IF oEval.FItemList(lp).isTempSoldOut Then response.write "class='soldOut'" %>>
									<div class="reviewBoxV15">
										...
										<div class="reviewTxt">
											<a href="" onclick="popEvaluateDetail(<%=oEval.FItemList(lp).Fitemid%>,<%=oEval.FItemList(lp).Fidx%>);return false;" title="상세 리뷰 보기"><% = chrbyte(oEval.FItemList(lp).Fcontents,70,"Y") %></a>
										</div>
										...
										<a href="" <%=chkIIF(oEval.FItemList(lp).FEvalCnt>0,"onclick=""popEvaluate('" & oEval.FItemList(lp).FItemid & "'); return false;""","")%> class="more1V15">상품 전체 리뷰보기</a>
										<div class="pdtPhoto">
											<a href="" onclick="popEvaluateDetail(<%=oEval.FItemList(lp).Fitemid%>,<%=oEval.FItemList(lp).Fidx%>); return false;"><img src="<%=getThumbImgFromURL(chkIIF(oEval.FItemList(lp).FImageIcon1<>"",oEval.FItemList(lp).FImageIcon1,oEval.FItemList(lp).FImageIcon2),400,400,"true","false")%>" alt="<%=replace(oEval.FItemList(lp).Fitemname,"""","")%>" /></a>
										</div>
									</div>
									<div class="pdtBox">
										<div class="pdtPhoto">
											<a href="/shopping/category_prd.asp?itemid=<%=oEval.FItemList(lp).Fitemid%>"><span <% IF oEval.FItemList(lp).isTempSoldOut Then response.write "class='soldOutMask'" %>></span><img src="<%=oEval.FItemList(lp).FImageList120%>" width="120px" height="120px" alt="<%=replace(oEval.FItemList(lp).Fitemname,"""","")%>" /></a>
										</div>
										<div class="pdtInfo">
					...
					<% else %>
						<div class="pdtWrap noReviewV15">
							<p><img src="http://fiximage.10x10.co.kr/web2015/shopping/best_review_none2.png" alt="" /></p>
							<p class="tPad15">앗! 작성된 포토후기가 없습니다.</p>
						</div>
					<% end if %>
