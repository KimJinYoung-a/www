...
%>
    <% If linkurl <> "" And imageurl <> "" Then %>
        <div style="position:absolute; right:10px; top:10px;">
            <a href="<%=linkurl%>" target="_blank" onclick="fnAmplitudeEventAction('click_loginpage_button','action','newmemberguide');"><img src="<%=staticImgUrl & "/main/" + db2Html(imageurl)%>" alt="<%=altname%>"></a>
        </div>
    <% End If %>
<% End If %>
<% on Error Goto 0 %>
