...
Dim image, image2, link, linktext, linktext2, startdate, enddate, linktext3
	...
	mainFile = "mainXMLBanner_691.xml"
...
If fileCont<>"" Then
	Response.Write "<ul class='topBnrListV15'>"
	...
	For each tplNodes in cTmpl
		...
		if i>5 then exit for	'//최대 6개까지 제한
...
%>
			...
			<li class="topBnr0<%=i+1%>">
			<% If InStr(link,"?") Then %>
				<a href="<%=link%>&gaparam=main_topbanner_2<%=i+1%>">
			<% Else %>
				<a href="<%=link%>?gaparam=main_topbanner_2<%=i+1%>">
			<% End If %>
					<p class="imgOverV15"><img src="<%=image%>" alt="<%=linktext%>" /></p><%' for dev msg : 배너 제목 alt값 속성에 넣어주세요 %>
