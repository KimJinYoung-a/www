...
Dim image, image2, link, linktext, linktext2, startdate, enddate, linktext3, linktext4
	...
	mainFile = "mainXMLBanner_689.xml"
...
If fileCont<>"" Then
	...
	For each tplNodes in cTmpl
		...
		linktext3	= tplNodes.getElementsByTagName("linktext3").item(0).text			'상세설명
	...
	<div class="bnrBasicV15 justDayLinkV15">
		<% If InStr(link,"?") Then %>
			<% If weekDay(now())="1" Or weekDay(now())="7" Then %>
				<a href="<%=link%>&gaparam=main_weekend_00">
			<% Else %>
				<a href="<%=link%>&gaparam=main_just1day_00">
			<% End If %>
		<% Else %>
			<% If weekDay(now())="1" Or weekDay(now())="7" Then %>
				<a href="<%=link%>?gaparam=main_weekend_00">
			...
			<% End If %>
		<% End If %>
			<p class="imgOverV15"><img src="<%=image%>" alt="<%=linktext2%>" /></p>
			<% If linktext2="" Or IsNull(linktext2) Then %>
				<% If weekDay(now())="1" Or weekDay(now())="7" Then %>
