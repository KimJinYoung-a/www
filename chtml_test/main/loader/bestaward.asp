...
<%
	...
	vRstCont = ""
	vXmlUrl = server.MapPath("/chtml/xml/mainBestAward.xml")
	...
	If vRstCont<>"" Then
		...
		For each subNodes in cSub
			...
			if subNodes.getElementsByTagName("addValue").item(0).text>0 Then
				...
				If InStr(link,"?") Then 
					vItem(lp) = vItem(lp) & "<a href="""&subNodes.getElementsByTagName("link").item(0).text&"&gaparam=main_award_"&si&""">" & vbCrLf
				Else
					vItem(lp) = vItem(lp) & "<a href="""&subNodes.getElementsByTagName("link").item(0).text&"?gaparam=main_award_"&si&""">" & vbCrLf
				End If
			...
			vItem(lp) = vItem(lp) & "										<strong></strong>" & vbCrLf
...
%>
				<div>
					<h2><img src="http://fiximage.10x10.co.kr/web2015/main/tit_bestaward.png" alt="BEST AWARD" /></h2>
					<ul class="mdListV15 bestMdV15">
						<%=vItem(0)%>
					</ul>
				</div>
