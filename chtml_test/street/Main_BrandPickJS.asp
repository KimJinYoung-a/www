...
Dim fnGetList
    ...
    If CInt(vImageCount) >= CInt(vTotalCount) Then
		...
		For i = 1 to vTotalCount
    		...
    		BufStr = BufStr + VarName + "+=" + DoubleQuat + "	<a href='"& wwwUrl & db2Html(fnGetList(1,i-1)) &"'><img src='" & staticImgUrl & "/brandstreet/main/" & db2Html(fnGetList(0,i-1)) &"' alt='브랜드Pick"&i&"' /></a>" + DoubleQuat + VbCrlf
...
''		End If
			...
			BufStr = BufStr + VarName + "+=" + DoubleQuat + "		<p class='brandPdt'><a href='"& wwwUrl & db2Html(fnGetList(1,i-1)) &"'>"&db2Html(fnGetList(2,i-1))&"</a></p>" + DoubleQuat + VbCrlf
