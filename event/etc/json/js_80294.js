...
$(function(){
	$.ajax({
		type: "get",
		url: "/event/etc/json/act_80294_thgv.asp",
		data: "pdv="+vPageDiv,
		cache: false,
			...
			if(typeof(message)=="object") {
				...
				if(typeof(message.giftlist)=="object") {
					...
					$(message.giftlist).each(function(){
						if(this.saleper!="") {
							$("#lyrItemList li .price").eq(i).html("<s>"+this.orgprice+"</s> "+this.sellprice+"<span>"+this.saleper+"</span>");
						} else {
							$("#lyrItemList li .price").eq(i).html(this.orgprice);
						}
