...
dim itemid, oJson, arrItem, strSort, sUnit, LoginUserid

arrItem = Request("arriid")
...
if arrItem<>"" then
	...
	if LoginUserid<>"" then
		...
		sqlStr = sqlStr & ", i.evalcnt , c.favcount , isNull(e.totalpoint,0) as totalpoint, f.itemid as favoriteitemid "
		...
		sqlStr = sqlStr & "LEFT JOIN [db_my10x10].[dbo].[tbl_myfavorite] as f "
		sqlStr = sqlStr & "ON i.itemid=f.itemid and f.userid='" & LoginUserid & "' "
		sqlStr = sqlStr & "where i.itemid in (" & arrItem & ") "
		sqlStr = sqlStr & "order by case " & strSort & " end "
	else
		sqlStr = "Select i.itemid, i.itemname, i.orgprice, i.sellcash, i.sailyn, i.basicimage, i.itemdiv "
		sqlStr = sqlStr & ", i.sellyn, i.limityn, i.limitno, i.limitsold , i.itemcoupontype , i.itemcouponvalue , i.itemcouponyn , i.brandname , i.optioncnt "
		sqlStr = sqlStr & ", i.evalcnt , c.favcount , isNull(e.totalpoint,0) as totalpoint, 0 as favoriteitemid "
		sqlStr = sqlStr & "from db_item.dbo.tbl_item as i "
		sqlStr = sqlStr & "INNER JOIN db_item.dbo.tbl_item_contents as c "
		sqlStr = sqlStr & "ON i.itemid = c.itemid "
		sqlStr = sqlStr & "LEFT OUTER JOIN db_board.dbo.tbl_const_eval_pointsummary as e "
	...
	if Not(rsget.EOF or rsget.BOF) then
		...
		Do Until rsget.EOF
			...
			oJson("items")(null)("favItemID") = rsget("favoriteitemid")
