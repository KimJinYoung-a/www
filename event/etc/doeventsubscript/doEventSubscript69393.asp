...
Dim vInstaxSt, vInstaxEd, chkProductCntToday, RvConNum
	...
	'// expiredate
	If not(left(Now(),10)>="2016-02-01" and left(Now(),10)<"2016-04-01") Then
	...
	If Not(rsget.bof Or rsget.Eof) Then
		...
		result2 = rsget(1) '// 당첨여부 0일 경우엔 비당첨, 1-당첨(라미 만년필)
	...
	If result1 = "" Then
		...
		If userBlackListCheck(userid) Then
			...
			Response.write "OK|<div class='result02'><div><img src='http://webimage.10x10.co.kr/eventIMG/2016/69393/img_win_coupon.png' alt='당첨이 안됐어요. 아쉽지만 3월 신규회원님을 위한 3종 쿠폰패키지를 발급해드렸어요!' /></div><a href='/my10x10/couponbook.asp' class='goCpbook'>MY TENBYTEN &gt; 쿠폰/보너스쿠폰</a></div>"
		...
		Else
			...
			If RvConNum >= vInstaxSt And RvConNum < vInstaxEd Then
				...
				'// 해당 유저의 로그값 집어넣는다.
				sqlstr = "INSERT INTO db_log.[dbo].[tbl_caution_event_log] (evt_code , userid , refip ,  value3, device)" + vbcrlf
				sqlstr = sqlstr & " VALUES("& eCode &", '"& userid &"' ,'"&refip&"', '라미 만년필 당첨', 'W')"
				dbget.execute sqlstr

				Response.write "OK|<div class='result01'><div><img src='http://webimage.10x10.co.kr/eventIMG/2016/69393/img_win_lamy.png' alt='라미 만년필 당첨' /></div><button class='btnConfirm' onclick='fnlayerClose();return false;'><img src='http://webimage.10x10.co.kr/eventIMG/2016/69393/btn_confirm.png' alt='확인' /></button></div>"
				dbget.close()	:	response.End
			Else
				'// 비당첨
				sqlstr = "INSERT INTO [db_event].[dbo].[tbl_event_subscript] (evt_code , userid , sub_opt1 , sub_opt2, regdate, device)" + vbcrlf
				...
				'// 해당 유저의 로그값 집어넣는다.
				sqlstr = "INSERT INTO db_log.[dbo].[tbl_caution_event_log] (evt_code , userid , refip ,  value3, device)" + vbcrlf
				sqlstr = sqlstr & " VALUES("& eCode &", '"& userid &"' ,'"&refip&"', '꽝', 'W')"
				dbget.execute sqlstr
