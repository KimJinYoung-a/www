...
Dim zipcode , usercell
	...
	if Not(Now() > #01/16/2017 00:00:00# And Now() < #01/22/2017 23:59:59#) then
		Response.Write "<script type='text/javascript'>alert('이벤트 기간이 아닙니다..'); top.location.href='/event/eventmain.asp?eventid="& eCode &"'</script>"
		dbget.close() : Response.End
	End If
	...
	If Not(GetLoginUserLevel = 3 Or GetLoginUserLevel = 4 Or GetLoginUserLevel = 6) Then
		Response.write "<script>alert('VIP 등급만 참여 하실 수 있습니다.'); top.location.href='/event/eventmain.asp?eventid="& eCode &"'</script>"
		dbget.close() : Response.End
	End If
