...
<%
dim editor_no , page ,i , idx_ix ,isMyComm
dim arrUserid, bdgUid, bdgBno
	page = getNumeric(requestCheckVar(request("page"),5))
	if page = "" then page = 1

	editor_no = getNumeric(requestCheckVar(request("editor_no"),5))
	isMyComm = requestCheckVar(request("isMC"),1)

	if editor_no = "" then
	response.write "<script>alert('에디터코드가 없거나 승인된 페이지가 아닙니다.');</script>"
	dbget.close()	:	response.End
	end if

dim oip_comment
	set oip_comment = new ceditor_list
	oip_comment.FPageSize = 10
	oip_comment.FCurrPage = page
	oip_comment.frecteditor_no = editor_no
	if isMyComm="Y" then oip_comment.frectUserid = GetLoginUserID
	oip_comment.feditor_comment_list()
%>
...
<div class="basicCmt">
	...
	<form name="frmcontents" method="post" action="/culturestation/culturestation_editor_process.asp">
	<input type="hidden" name="idx">
	<input type="hidden" name="mode">
	<input type="hidden" name="editor_no" value="<%= editor_no %>">
		<p class="cmtInpWrap" style="height:98px;"><textarea name="comment" onClick="jsCheckLimit();" onKeyUp="jsCheckLimit();" <%IF NOT(IsUserLoginOK) THEN%>readonly<%END IF%>><%IF NOT IsUserLoginOK THEN%>로그인 후 글을 남길 수 있습니다.<%END IF%></textarea></p>
		<div class="note01 overHidden tPad10">
			...
			<a href="" class="ftRt btn btnW130 btnS1 btnRed" onclick="reg();return false;" >코멘트 남기기</a>
		</div>
	</form>
	</div>
	...
	<div class="basicCmtList tMar40">
		...
		<table class="tMar10">
			...
			<tr>
				...
				<td class="colCont"><%= nl2br(oip_comment.FItemList(i).fcomment) %></td>
