	...
	$(function(){
		...
		$(window).scroll(function(){
			if( $(window).scrollTop()==($(document).height()-$(window).height()) ) {
				...
				var linkurl;
				if($("#more").val()=="Y"){
					linkurl="_act_culturestation_event_more.asp";
					$("#page2").val(Number($("#page2").val())+1);
				}else{
					linkurl="_act_culturestation_event.asp";
				}
		...
		<div id="contentWrap">
			<div class="cult-head">
				...
				<ul class="nav">
					...
					<li class="all<%=chkIIF(etype=""," on","")%>"><a href="/culturestation/_index.asp">전체</a></li>
					<li class="feel<%=chkIIF(etype="0"," on","")%>"><a href="/culturestation/_index.asp?etype=0&sort=<%=sortMtd%>&mylist=<%=mylist%>">느껴봐</a></li>
					<li class="read<%=chkIIF(etype="1"," on","")%>"><a href="/culturestation/_index.asp?etype=1&sort=<%=sortMtd%>&mylist=<%=mylist%>">읽어봐</a></li>
				</ul>
			</div>
			<div class="cult-conts">
				...
				<%
							...
							If i=4 Or (oevent.FResultCount<5 And i=(oevent.FResultCount-1)) Then
								Server.Execute("_main_cultureStationBox.asp")
							End If
						Next
					Else
