...
<%
'#######################################################
' Discription : pc_right_search_banner // cache DB경유
' History : 2019-02-15 원승현 생성
'           2019-03-12 검색어기반 배너 출력을 왼쪽에 표시로 변경(원래 왼쪽에 표시되던 배너 기능(include_left_search_banner.asp는 사용안함)
'#######################################################
Dim intI
Dim sqlStr , rsMem , arrList, poscode, SearchText
Dim gaParam : gaParam = "gaparam=top_search_banner_" '//GA 체크 변수

'//DB 시간 타이머 00시부터 00시 5분 사이에는 DB 캐쉬 1분에 한번 읽기
Dim cTime , dummyName
If timer > 10 And Cint(timer/60) < 6 Then
	cTime = 60*1
	dummyName = "MBRSBAN_"&Cint(timer/60)
Else
	cTime = 60*1
	dummyName = "MBRSBAN_"
End If

'SearchText = request("rect")
SearchText = replace(requestCheckVar(request("rect"),100),"%27","")
...
sqlStr = sqlStr & "	    AND itemDesc LIKE '%"&SearchText&"%' " & vbcrlf
sqlStr = sqlStr & " ORDER BY orderidx ASC, idx DESC "

set rsMem = getDBCacheSQL(dbget, rsget, "MAINRIGHTSEARCHBAN", sqlStr, cTime)
...
If IsArray(arrList) Then
    ...
    End If
%>
    <% If instr(itemDesc, SearchText) > 0 Then %>
