...
<ul id="lyrSchExpTab" class="schTabV15">
	<% if catTTCnt>0 then %>
	<li class="tabCtgy <%=chkIIF(makerid="","current","")%>" name="Cate">
		<p>
			<strong>아이템</strong>
	...
	<% end if %>
	<% if brnTTCnt>0 then %>
	<li class="tabBrand <%=chkIIF(makerid<>"","current","")%>" name="Brand">
		<p>
			<strong>브랜드</strong>
...
'// 카테고리 확장검색 //
	...
	if oGrCat.FResultCount>0 then
		...
		for Lp=0 to oGrCat.FResultCount-1
			...
			'2dep 내용 삽입
			if oGrCat.FItemList(lp).FCateCd2<>"" then
