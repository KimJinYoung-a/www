 ...
 * 
 * 현재 회원이 Biz회원이 아닌데 Biz상품일경우 sell_price가 null로 떨어짐
 *  => BIZ회원전용공개로 가격 표시
 */
Vue.component('PRODUCT-INFO', {
    template : `
        <div class="pdtInfo">
            <p class="pdtBrand tPad20"><a href="javascript:void(0);" @click="go_brand_detail">{{brand_name}}</a></p>
            <p class="pdtName tPad07"><a @click="go_product_detail">{{item_name}}</a></p>
            <p v-if="sell_price && (sale_yn || coupon_yn)" class="pdtPrice"><span class="txtML">{{number_format(org_price)}}원</span></p>

            <p v-if="sell_price">
                <span class="finalP">{{number_format(this.sell_price)}}원</span>
                <strong v-if="sale_percent > 0 && (sale_yn || coupon_yn)" :class="sale_percent_class">[{{sale_percent}}%]</strong>
            </p>
            <p v-else class="PdtPriceHide">BIZ 회원전용 공개</p>
    ...
    props: {
        ...
        sell_price : { type : Number, default : null }, // 판매 가격
