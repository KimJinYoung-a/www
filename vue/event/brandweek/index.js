const app = new Vue({
    el: '#app',
    template: `
      <div class="evt117084 brand_week">
       <section class="section01">
        ...
        <template v-for="(item,index) in conceptItems">
           <section :class="'section0' + (index+3)">
            <p class="tit">
                <img :src="commonImagePath + 'badge0' + (index+1) + '.png' + imgVer" alt="">
            </p>     
            <div :class="'main_prd conceptItems'+item.mainItemId">
             <div class="thumbnail"><img src="//thumbnail.10x10.co.kr/webimage/image/basic600/210/B002105986.jpg" alt=""></div>
             <div class="desc">
              <div class="price"><s>15,000</s> <span class="sale">30%</span>11,000</div>
             </div>
             <a href="javascript:void(0)" class="btn_purchase" @click="prdPage(item.mainItemId, item.mainItemPopup)"></a>
            </div>
            <ul id="lyrItemlist" class="sub_prd">
            
                    <li v-for="(sub,index2) in item.subItems">
                        <a href="javascript:void(0)" @click="prdPage(sub.itemid,sub.popup)" :class="'conceptSubItems'+sub.itemid">
                            <div class="thumbnail">
       ...
       <section class="section07 brand_wrap">
        <p class="coupon"></p>
        <div class="event">
          <div class="event_wrap">
              <template v-for="event in linkEvents">
                  <a :href="'/event/eventmain.asp?eventid='+event.id" v-if="event.id"></a>            
              </template>
          </div>
        </div>
        <div class="brand">
          <div class="brand_wrap">
              <template v-for="brand in linkBrands">
                  <a :href="'/street/street_brand_sub06.asp?makerid='+brand.id" v-if="brand.id"></a>            
              </template>
          </div>
        </div>
        
       </section>
       <section class="section08 popup" v-show="productPopup">
        <div class="bg_dim"></div>
        <div class="pop">
         <img src="//webimage.10x10.co.kr/fixevent/event/2022/119567/popup.png" alt="">
         <a href="javascript:void(0)" class="btn_close" @click="prdPopup()"></a>
         <a href="javascript:void(0)" class="btn_prd" @click="prdPage('',false)"></a>
        </div>
       </section>
...
<!--                        <span><b>12.7</b> 화요일</span> 제품 미리보기-->
      ...
      <!-- //MD HITCHHIKER vol.87 (W) 111165 -->
    `,
    data: function () {
        return {
            eventCode: eCode,
            currentDate: isStaging ? '20220228' : this.getToday(),
            ...
            infoPopupItemid: '',
            productPopup : false,
            brandId : event_data.brandId,
            eventName : event_data.eventName,
            linkBrandId : event_data.linkBrandId,
            linkEvents : event_data.linkEvents,
            linkBrands : event_data.linkBrands
        }
    },
    created() {
        // 타임딜 세팅
    ...
    methods : {
         ...
         * @param itemid
         */
        prdPage(itemid, popup) {
            if(itemid == '') {
                itemid = this.infoPopupItemid;
            }

            if (this.hideItems(itemid)) {
              this.prdPopup();
              return;
            }

            if(popup) {
                this.infoPopupItemid = itemid;
                this.prdPopup();
                return;
            } else {
                this.infoPopupItemid = '';
            }
        ...
        hideItems(itemId) {
          const _this = this;
          const hideItemIds = ["4813511", "4813509", "4813507", "4813505", "4818033", "4818034"];
          let result = false;
          hideItemIds.forEach(function(i) {
              if (i === itemId && _this.isShowDate()) {
                  result = true;
              }
          })
          return result;
        ...
        isShowDate() {
            ...
            const openDate = new Date(2022, 7, 16, 00, 00, 00);
            if (todayDate.getTime() < openDate.getTime()) {
                return true;
            } else {
                return false;
            }
