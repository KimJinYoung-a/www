					...
					$(message.items).each(function(){
						...
						if(isPrc){
							if(isSale){
								//할인율 표시
								if(this.saleper!="") {
									var saleCouponTag = (this.saleTag == "coupon" ? "class='sale'" : "")

									if (this.itemdiv == "21") {
										if (opts.saleBracket) {
											$("."+opts.target+this.itemid+" .price").html(this.sellprice+"~ <span class='sale'>["+this.saleper+"]</span>");
										} else {
											$("."+opts.target+this.itemid+" .price").html(this.sellprice+"~ <span class='sale'>"+this.saleper+"</span>");
										}
									} else {
										if (opts.saleBracket) {
											$("."+opts.target+this.itemid+" .price").html("<s>"+this.orgprice+"</s> "+this.sellprice+"<span class='sale'>["+this.saleper+"]</span>");
										} else {
											$("."+opts.target+this.itemid+" .price").html("<s>"+this.orgprice+"</s> "+this.sellprice+"<span class='sale'>"+this.saleper+"</span>");
										}
									}
