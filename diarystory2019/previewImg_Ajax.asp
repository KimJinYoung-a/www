...
' History : 2018-08-23 일부 변경 기존 로직 재사용
'####################################################
%>
...
<% If odibest.FTotalCount > 0 Then %>
	<h3><img src="http://fiximage.10x10.co.kr/web2018/diary2019/tit_diary_story_2.png" alt="DIARY PREVIEW" /></h3>
	<div class="slide">
		<%' 이미지 사이즈 670*470 %>
	...
	<div class="pdtInfo">
		...
		<% If odibest.FItemList(0).IsSaleItem or odibest.FItemList(0).isCouponItem Then %>
			<% If odibest.FItemList(0).IsSaleItem Then %>
				<p class="price"><%=FormatNumber(odibest.FItemList(0).getRealPrice, 0) %>원 <strong class="color-red">[<%=odibest.FItemList(0).getSalePro%>]</strong></p>
			<% End If %>
			<% IF odibest.FItemList(0).IsCouponItem Then %>
				<p class="price"><%=FormatNumber(odibest.FItemList(0).GetCouponAssignPrice, 0) %>원 <strong class="color-green">[<%=odibest.FItemList(0).GetCouponDiscountStr%>]</strong></p>
			<% End If %>
		<% Else %>
			<p class="price"><%=FormatNumber(odibest.FItemList(0).getRealPrice, 0) %>원</p>
		<% end if %>
	</div>
	<a href="/shopping/category_prd.asp?itemid=<%=odibest.FItemList(0).FItemid%>" onclick="fnAmplitudeEventAction('click_diary_moreitem_button','itemid','<%=odibest.FItemList(0).FItemid%>');" class="btn-detail btnV18 btn-pink" target="_blank">상품 상세보기</a>
<% End If %>
