...
%>
			...
			<section class="sect-evt">
				<h2><a href="/diarystory2022/exhibition.asp">주목해야 할 기획전</a></h2>
				<div class="evt-list">
					<% If mktevent.FOneItem.FEvt_code <> "" Then %>
					<div class="dr-evt-wrap dr-evt-mkt">
                        <article class="dr-evt-item">
                            ...
                            <a href="/event/eventmain.asp?eventid=<%=mktevent.FOneItem.FEvt_code%>" class="dr-evt-link" onclick="fnAmplitudeEventAction('click_diarystory_event','event_code','<%=mktevent.FOneItem.FEvt_code %>');"><span class="blind">이벤트바로가기</span></a>
                        </article>
					</div>
                    <% end if %>
					...
					<div class="dr-evt-wrap">
                        <article class="dr-evt-item<% if itemCheck = "" then %> dr-evt-mkt<% end if %>">
                            ...
                            <a href="/event/eventmain.asp?eventid=<%=hoteventlist.FItemList(i).fevt_code %>" class="dr-evt-link" onclick="fnAmplitudeEventAction('click_diarystory_event','event_code','<%=hoteventlist.FItemList(i).fevt_code %>');"><span class="blind">이벤트바로가기</span></a>
                        </article>
						<div class="prd-list">
                            <%=itemCheck%>
						</div>
