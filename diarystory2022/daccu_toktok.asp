...
$(function(){
	fnAmplitudeEventAction('view_diarystory_daccutalktalk','','');
	// MD 롤링
...
function getUserDaccuMasterList() {
    $.ajax({
        type:"GET",
        url:"/diarystory2022/lib/act_daccu_toktok_usermasterlist.asp",
...
function fnDeleteDaccu(midx) {
	if (confirm('등록하신 다꾸톡톡을 삭제하시겠습니까?')) {
		$.ajax({
            type:"GET",
            url:"/diarystory2022/lib/ajaxDaccuTokTok.asp?daccuTokMode=daccuDelete&daccuTokMasterIdx="+midx,
            //data: ,
            dataType: "text",
            async:false,
            cache:true,
...
function daccutoktokView(midx) {
	fnOpenModal('/diarystory2022/daccu_toktok_view.asp?masterIdx='+midx);
	popupShow();
}
...
function daccuTokWrite() {
	<% If IsUserLoginOK() Then %>
		fnOpenModal('/diarystory2022/daccu_toktok_write.asp');
		popupShow();	
    <% Else %>
