...
class cdiary_list
	...
	public Function fnGetdievent()
       Dim sqlStr ,i
		...
		dim rsMem : set rsMem = getDBCacheSQL(dbget,rsget,"NEWDIEVCNT",sqlStr,180)
		...
		If FTotalCount > 0 Then
			...
			'rsget.LockType = adLockOptimistic
			'rsget.Open sqlStr, dbget
			set rsMem = getDBCacheSQL(dbget,rsget,"NEWDIEV",sqlStr,180)
            if (rsMem is Nothing) then Exit Function ''추가
			...
			if  not rsMem.EOF  then
				...
				do until rsMem.eof
					...
					FItemList(i).fisNew		= rsMem("isNew")
...
Public Function fngetDiaryEvtItemHtml(eventid)
	...
	For intLoop =0 To UBound(arrListItem,2)
		link = "/shopping/category_Prd.asp?itemid="& arrListItem(0,intLoop) &""
		...
		resultHtml = resultHtml &"	<article class=""prd-item"">"
		resultHtml = resultHtml &"		<figure class=""prd-img"">"
		...
		resultHtml = resultHtml &"		<div class=""prd-info"">"
		resultHtml = resultHtml &"			<div class=""prd-price"">"
		resultHtml = resultHtml &"				<span class=""set-price""><dfn>판매가</dfn>" & price & "</span>"
		resultHtml = resultHtml &"				<span class=""discount""><dfn>할인율</dfn>" & discount & "</span>"
		resultHtml = resultHtml &"			</div>"
		resultHtml = resultHtml &"			<div class=""prd-badge"">"
		'if fnGetGiftiCon(arrListItem(13,intLoop),arrListItem(4,intLoop),arrListItem(0,intLoop)) then
		'resultHtml = resultHtml &"				<i class=""badge-gift"">선물</i>"
		'End If
		if fnGetDeliveryFreeiCon(arrListItem(13,intLoop),arrListItem(4,intLoop),arrListItem(14,intLoop)) then
		resultHtml = resultHtml &"				<i class=""badge-delivery"">무료배송</i>"
		End If
		resultHtml = resultHtml &"			</div>"
		resultHtml = resultHtml &"		</div>"
		resultHtml = resultHtml &"		<a href="""& link &""" class=""prd-link""><span class=""blind"">상품 바로가기</span></a>"
		resultHtml = resultHtml &"	</article>"
		resultHtml = resultHtml &"	<a href=""/event/eventmain.asp?eventid=" & eventid & """ class=""btn-more"" onclick=""fnAmplitudeEventMultiPropertiesAction('click_diarystory_event_item','event_code|item_id','" & eventid & "|" & arrListItem(0,intLoop) & "');""><i></i>더보기</a>"
	Next
	End If
