...
%>
								...
								<tr>
									<td colspan="9">
										...
										<div class="orderTotal">
											<span>품절 취소 상품수 <strong><%= vItemKindCnt %>종(<%= FormatNumber(vItemNoCnt, 0) %>개)</strong></span>
											<span>&nbsp;</span>
											<span>품절 취소 상품 총액 <strong><%= FormatNumber(vItemReducedPriceSUM, 0) %>원</strong></span>
											<%
											if (stockoutBeasongPay > 0) then
												vItemReducedPriceSUM = vItemReducedPriceSUM + stockoutBeasongPay
											%>
											<span>&nbsp;</span>
											<span>품절 취소 배송비 <strong><%= FormatNumber(stockoutBeasongPay, 0) %>원</strong></span>
											<% end if %>
										</div>
										<% end if %>
									</td>
